...
- Entity 保持纯数据对象，不定义 `validate()`，也不混入业务校验能力。
- 所有业务字段校验均放入对应的 ViewModel validation mixin；不得在 Entity
  或 Repository 中新增、恢复或间接包装纯字段校验。
...
- ViewModel 通过模块 validation mixin 执行字段值域、枚举、Flags 和跨字段
  规则；通用断言复用 `ViewModelValidationMixin`，不要把规则放回 Entity。
...
- Repository 提供 ViewModel 执行唯一性、外键和孤儿数据校验所需的查询能力，
  并负责删除引用保护；不重复承担 ViewModel 的纯字段校验。
...
| `player_create_info` | `playercreateinfo` 8 列；详情页全部 5 个关联 Tab：`playercreateinfo_action` 5 列、`playercreateinfo_item` 5 列、`playercreateinfo_skills` 5 列、`playercreateinfo_spell_custom` 4 列、`playercreateinfo_cast_spell` 4 列，共 31 个物理列 | Foxy `afb4e72a049707637a56ad0b5eaf1be65b526358`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；3.3.5a DBC；实际 `acore_world` / `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块 Entity/ID/Mask/动作类型/值域/父项筛选/Picker/布局契约测试 8 项通过；6 张表 Repository 非空/空表及通配、按位筛选冒烟通过；`git diff --check` 通过 | 2026-07-14 | 补齐原详情页遗漏的初始技能和登录施法 Tab；31 个物理列均为独立标量字段和显式 Controller。`race/class` 按 ChrRaces/ChrClasses 标量 ID 校准，三张 Mask 表按各自 loader 的位掩码与 `0=全部` 语义校准，可玩总掩码分别为 1791/1535，不复用其他表的同名常量；动作类型精确使用 `Player.h::ActionButtonType`，法术/物品动作使用对应 Picker；出生地图排除 core 不接受的实例地图，全部表单行四列等宽。实测 6 张表结构与 core 一致，记录数为 62/283/1/77/0/2，31 个字段值域无异常，通配物品和按位技能/法术父项筛选正确，登录施法无重复组合且法术引用完整。未新增 required DBC；实测 `foxy.dbc_skill_line` 尚未导入，技能 Picker 与技能外键数据核验需用户重新导入既有 required `SkillLine.dbc` 后可用。Windows UI 手工点击未验证。全量 `flutter test` 为 162 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `area_table` | `foxy.dbc_area_table` 36 列；详情页无关联 Tab；新增 6 张引用 DBC 镜像共 100 个物理列 | Foxy `04c6b0a8a2871c6420ee10aafde2b543082d2895`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / WoWDBDefs 3.3.5.12340；实际 `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/AreaTeams/AreaFlags/值域/Picker/布局及 DBC definition/字段/导出契约测试 15 项通过；2307 条全量 Entity 校验及 Repository 分页、计数、单条回读、新建编号/探索位冒烟通过；`git diff --check` 通过 | 2026-07-14 | 36 个主表列和新增 DBC 的 100 个列均为独立标量字段。`AreaBit` 按 `PLAYER_EXPLORED_ZONES_SIZE=128` 校准为唯一的 `0..4095` 探索位索引，复制时重新分配；`FactionGroupMask` 精确使用 `AreaTeams` 的 `0/2/4/6`，不复用 `FactionMasks`；`Flags` 使用当前 core 的 AreaFlags，并保留实际 3.3.5a DBC 普遍存在但 core 未命名的 `0x40000000` 客户端位。地图、父区域、声音提供器、环境声音、区域音乐、进入音乐、液体类型和光照均使用各自准确 Picker，液体槽按水/海洋/岩浆/软泥区分，全部表单行四列等宽。新增 required `SoundProviderPreferences.dbc`、`SoundAmbience.dbc`、`ZoneMusic.dbc`、`ZoneIntroMusicTable.dbc`、`LiquidType.dbc`、`Light.dbc` 全标量 Entity/Repository/Picker/导出闭环；现有用户需重新导入 DBC，实测 `foxy` 库尚无这 6 张镜像表。实测 2307/2307 行通过当前语义校验，父区域无孤立/自引用、探索位全部唯一；区域 676 的 `ContinentID=150` 在现有 `dbc_map` 镜像中缺失，属于客户端既有数据/镜像差异。Windows UI 手工点击未验证。全量 `flutter test` 为 169 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `emote_text` | `foxy.dbc_emotes_text` 19 列；详情页无关联 Tab；新增引用 `foxy.dbc_emotes_text_data` 18 列 | Foxy `e6aa95a998eac6f54a02bf5559e4281bef2058cd`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / WoWDBDefs 3.3.5.12340；实际 `acore_world` / `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/引用/值域/Picker/布局及 DBC definition/本地化/字段/导出契约测试 29 项通过；252 条全量 Entity 校验及 Repository 分页、计数、单条回读、新建编号冒烟通过；`git diff --check` 通过 | 2026-07-14 | 19 个主表列和新增 DBC 的 18 个列均为独立标量字段，View/ViewModel 不再通过循环或索引分派管理 `EmoteText0..15`，全部表单行四列等宽。`EmoteID` 按 WoWDBDefs 精确指向 `Emotes.dbc`，16 个 `EmoteText` 槽位精确指向 `EmotesTextData.dbc`，不误用同名 `EmotesText.dbc` 或枚举；AzerothCore 服务端只加载前者并跳过 16 个客户端文本引用列。新增 required `EmotesTextData.dbc` 全标量 Entity/Repository/Picker/本地化/导出闭环，现有用户需重新导入 DBC；实测 `foxy` 库尚无 `dbc_emotes` 与 `dbc_emotes_text_data`，故引用目标存在性需导入后复核。实测镜像表 252/252 行通过值域校验，世界库 fallback `emotestext_dbc` 结构与 core 一致且当前为空。Windows UI 手工点击未验证。全量 `flutter test` 为 177 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `quest_faction_reward` | `foxy.dbc_quest_faction_reward` 11 列；详情页无关联 Tab | Foxy `2f00e34662c1970e0843793d92034bbcfd0653f1`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / WoWDBDefs 3.3.5.12340；实际 `acore_world` / `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/固定 ID/符号语义/值域/布局和 Repository 契约测试 7 项通过，连同 DBC 字段/definition/导出契约共 15 项通过；2 条全量 Entity 校验及 Repository 分页、计数、单条回读、固定 ID 和禁止复制/删除冒烟通过；`git diff --check` 通过 | 2026-07-14 | 11 个物理列均为独立标量字段，保留数据库物理列名 `Difficulty0..9`，UI 按实际服务端语义修正为索引 0..9 的声望值，全部表单行四列等宽。AzerothCore 通过任务模板 `RewardFactionValue` 的正负号选择固定记录 ID 1/2，再以绝对值 0..9 选取奖励值，因此本模块不使用难度枚举或 Entity Picker；ID 1/2 分别校验为非负/非正奖励表，禁止 MAX+1、复制和删除。实测镜像表 2/2 行通过校验，9464 条任务模板的实际索引均在 -9..9，世界库 fallback `questfactionreward_dbc` 结构与 core 一致且当前为空；未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 184 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `quest_sort` | `foxy.dbc_quest_sort` 18 列；详情页无关联 Tab；关联 `quest_template.QuestSortID` | Foxy `583a2791be5d7daf9b100698b0db5fce792b0725`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / WoWDBDefs 3.3.5.12340；实际 `acore_world` / `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/DBC 格式/本地化/值域/引用符号/布局和删除保护契约测试 7 项通过，连同 DBC definition/字段/本地化 codec/导出契约共 30 项通过；41 条全量 Entity 校验及 Repository 分页、计数、单条回读、16 语言槽、新建编号和引用删除保护冒烟通过；`git diff --check` 通过 | 2026-07-14 | 18 个物理列均为独立标量字段，16 个名称字段通过固定 DBC locale 编辑器管理，`SortName_lang_Flags` 作为独立 signed int32 物理字段显式编辑，详情表单行四列等宽。任务模板以正 `QuestSortID` 引用 AreaTable、以负值 `-QuestSort.ID` 引用 QuestSort，选择器已按此符号规则保存；QuestSort ID 限制在可由 signed smallint 负值表示的 1..32768，仍被任务引用的记录禁止删除。AzerothCore 的 `QuestSort` 枚举只是 41 个已知分类及职业/技能特殊分支，不作为另一个字段枚举下拉。实测镜像表 41/41 行通过校验，9464 条任务中 2122 条使用 39 个 QuestSort 分类且无缺失引用；世界库 fallback `questsort_dbc` 为 core 兼容布局且当前为空。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 191 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `quest_info` | `foxy.dbc_quest_info` 18 列；详情页无关联 Tab；关联 `quest_template.QuestInfoID` | Foxy `bc43700bfa0b7a05dfa6ca95da4b002e4a02b55f`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / WoWDBDefs 3.3.5.12340；实际 `acore_world` / `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/DBC 格式/本地化/值域/类型来源/布局和删除保护契约测试 7 项通过，连同 DBC definition/字段/本地化 codec/导出契约共 30 项通过；11 条全量 Entity 校验及 Repository 分页、计数、单条回读、16 语言槽、新建编号和引用删除保护冒烟通过；`git diff --check` 通过 | 2026-07-14 | 18 个物理列均为独立标量字段，16 个名称字段通过固定 DBC locale 编辑器管理，`InfoName_lang_Flags` 作为独立 signed int32 物理字段显式编辑，详情表单行四列等宽。`QuestInfoID` 对应 QuestInfo DBC 行和 `SharedDefines.h::QuestTypes`，与 `QuestType` 对应的 `Quest::Method` 0/1/2 严格分离；任务模板使用 QuestInfo Picker，不复用 Method 枚举。AzerothCore 当前不加载 QuestInfo DBC Store，也没有世界库 fallback 表，但会将该值作为 `Quest::Type` 用于 PvP/团队判断并发送给客户端；因此允许 1..65535 的自定义 DBC ID，不把 11 个已知行伪造成行内枚举，仍被任务引用的记录禁止删除。实测镜像表 11/11 行与核心已知 QuestTypes 一致，9464 条任务中 1641 条使用 10 个类型且无缺失引用。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 198 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `item_extended_cost` | `foxy.dbc_item_extended_cost` 16 列；详情页无关联 Tab；新增引用 `foxy.dbc_item_purchase_group` 26 列；关联 `npc_vendor.ExtendedCost`、`game_event_npc_vendor.ExtendedCost` | Foxy `6f969622c518789d68e6a3ab2145f0eca758df12`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / WoWDBDefs 3.3.5.12340；实际 `acore_world` / `foxy` 库经 Laconic + laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/ArenaSlot/值域/槽位规则/Picker/布局/删除保护及 DBC definition/本地化/字段/导出契约共 30 项通过；972 条全量 Entity 校验及 Repository 分页、计数、单条回读和新建编号冒烟通过；`git diff --check` 通过 | 2026-07-14 | 主表 16 列和新增 DBC 26 列均为独立标量字段，Entity/ViewModel 未使用数组、Map、索引分派或循环管理重复槽位；详情表单全部四列等宽，5 个物品 ID 使用 `item_template` Picker。`ArenaBracket` 按 `ItemExtendedCostEntry::reqarenaslot` 和 `ArenaSlot` 精确校准为 0/1/2，不复用值为 2/3/5 的 `ArenaTeamTypes`。`ItemPurchaseGroup` 在 AzerothCore `ItemExtendedCostEntryfmt` 中被跳过，warcrafty 则明确提供独立 `ItemPurchaseGroup.dbc` 结构，因此使用独立 DBC Picker 而非同名枚举；新增 required `ItemPurchaseGroup.dbc` 全标量 Entity/Repository/Picker/本地化/导出闭环，现有用户需重新导入 DBC，实测库尚无该镜像表。实测 972/972 行通过校验，ArenaSlot 分布为 955/16/1，全部物品引用完整；3 个 ItemID 为 0 的高位空槽保留非零 Count，符合 core 仅在 ItemID 非零时读取数量的行为。13,370 条商人和 63 条活动商人非零扩展价格引用均无断链，世界库 fallback 表为空。Windows UI 手工点击未验证。全量 `flutter test` 为 205 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `scaling_stat_distribution` | `foxy.dbc_scaling_stat_distribution` 22 列；详情页无关联 Tab；关联 `item_template.ScalingStatDistribution` | Foxy `5f3302c96639f4f491230a0b3dfbdca9dc826294`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/属性类型来源/空槽/值域/标量管理/布局/单 Repository/删除保护及 DBC definition/字段/导出契约共 16 项通过；58 条全量 Entity 校验及 Repository 分页、计数、单条回读、新建编号冒烟通过；`git diff --check` 通过 | 2026-07-14 | 22 个物理列均为独立标量字段，Entity/ViewModel 未使用数组、Map、索引分派或循环管理 10 组属性槽；详情页每行四列等宽，并将两组“属性类型+缩放系数”配对展示，未新增说明文本。`StatID0..9` 按 `ScalingStatDistributionEntry::StatMod` 到 `Player::_ApplyItemBonuses` 的直接调用链校准：`-1` 为本表空槽，非负值进入 `ItemModType` switch；排除无定义的 2/8/9/10/11，同时保留实际客户端 DBC 中存在但当前 core 不应用的 40。`Bonus0..9` 是以 10000 为分母的无符号缩放系数，允许实际数据存在的零值；`Maxlevel` 按 core 的等级截断语义保留 0。合并原重复 Repository，修复预分配 ID 被覆盖及新建日志 ID 错误，并保护仍被物品模板引用的记录。实测 58/58 行通过校验，40 个非零物品引用均完整，2 个 StatID 40、4 行 Maxlevel 0 和 53 个非负类型/零系数槽按运行时行为保留；世界库 fallback `scalingstatdistribution_dbc` 结构与 core 一致且当前为空。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 213 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `spell_item_enchantment` | `foxy.dbc_spell_item_enchantment` 38 列；详情页无关联 Tab；新增直接引用 `foxy.dbc_item_visuals` 6 列、`foxy.dbc_item_visual_effects` 2 列和 `foxy.dbc_spell_item_enchantment_condition` 31 列 | Foxy `63442699c1e789eb975a49e59b72785ee70311cf`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/联合效果/专属枚举/Flags/Picker/标量管理/四列布局/单 Repository/引用保护及 DBC definition/字段/导出契约测试通过；2,656 条全量 Entity 校验及 Repository 分页、计数、新建编号冒烟通过；`git diff --check` 通过 | 2026-07-14 | 主表 38 列和新增 DBC 39 列均为独立标量字段。三组 `Effect/EffectPointsMin/EffectPointsMax/EffectArg` 按 `ItemEnchantmentType` 0..8 和 `Player::ApplyEnchantment` 的实际分支校准：法术、`SpellSchools`、附魔专用 `ItemModType` 分别使用准确 Picker/独立枚举，不复用名称相似的普通物品枚举；core 跳过但客户端保留的 `EffectPointsMax`、未使用槽原始值、负伤害、`ItemVisual=-1` 和 Flags 2/4/8 位均不被清零。全部表单行四列等宽，未新增说明文本；统一 Repository 修复预分配 ID 二次改号，并保护物品 socket、宝石、随机属性/后缀、法术效果和 proc 引用。新增 required `ItemVisuals.dbc`、`ItemVisualEffects.dbc`、`SpellItemEnchantmentCondition.dbc` 全标量 Entity/Repository/Picker/导出闭环，现有用户需重新导入 DBC；实测 `foxy` 库尚无这 3 张镜像表。实测 2,656/2,656 行通过校验；59 个效果法术在现有 `dbc_spell` 镜像中缺失，另有 2 条宝石和 1 条法术反向引用缺失，属于既有数据/镜像同步问题。Windows UI 手工点击未验证。全量 `flutter test` 为 221 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `gem_property` | `foxy.dbc_gem_properties` 5 列；详情页无关联 Tab；关联 `foxy.dbc_spell_item_enchantment.ID` 和 `item_template.GemProperties` | Foxy `d45efd4aaf3e43f7153ed128c9d0a7d85ef790bb`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/专属颜色组合/值域/Picker/四列布局/引用校验/删除保护及 DBC definition/字段/导出契约共 15 项通过；626 条全量 Entity 校验及 Repository 分页、计数、新建编号冒烟通过；`git diff --check` 通过 | 2026-07-16 | 5 个物理列均为独立标量字段。`Type` 按 `GemPropertiesEntry::color` 和 `SocketColor` 的实际插槽判断校准为多彩、三原色、橙/紫/绿和棱彩 8 种本表专属位组合，不复用普通物品 socket 单色枚举；`Enchant_ID` 使用法术物品附魔 Picker。AzerothCore 的 `GemPropertiesEntryfmt="nixxi"` 明确跳过 `Maxcount_inv` 与 `Maxcount_item`，因此二者保留为可编辑客户端物理列，只校验非负 int32，不伪造服务端跨字段约束。Repository 修复预分配 ID 和新建日志类型，禁止新增断链附魔，同时允许既有断链原值不变，并保护仍被物品模板引用的记录；详情表单全部四列等宽，未新增说明文本。实测 626/626 行通过校验，颜色仅出现上述 8 种组合；611 个非零物品模板引用全部完整，11 条零附魔合法保留，ID 1142/1143 的附魔 3372/3373 在现有镜像中缺失，属于既有数据问题；世界库 fallback `gemproperties_dbc` 结构与 core 一致且当前为空。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 228 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `glyph_property` | `foxy.dbc_glyph_properties` 4 列；详情页无关联 Tab；关联 `foxy.dbc_spell.ID`、`foxy.dbc_spell_icon.ID`、法术效果 74 和角色库 `character_glyphs.glyph1..6` | Foxy `f5e735851403a6c9889faf7e34a2450fa0edf40a`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `acore_characters` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/专属类型/值域/Picker/四列布局/引用校验/删除保护及 DBC definition/字段/导出契约共 15 项通过；365 条全量 Entity 校验及 Repository 分页、计数、新建编号、跨库 QueryBuilder 冒烟通过；`git diff --check` 通过 | 2026-07-16 | 4 个物理列均为独立标量字段。`GlyphSlotFlags` 按 `GlyphPropertiesEntry::TypeFlags` 与 `GlyphSlotEntry::TypeFlags` 的精确相等判断校准为本表专属 `0=小型`、`1=大型`，不误用 Flags 控件，也不把它当作 `GlyphSlot.ID`；法术和法术图标分别使用准确 DBC Picker。ID 按角色表 `smallint unsigned` 与客户端协议 `uint16` 限制为 1..65535；新建必须引用存在的法术，图标允许 0。Repository 修复预分配 ID 和新建日志类型，禁止新增断链引用及非 0/1 类型，同时允许既有异常原值不变，并保护法术效果 74 与可发现角色库的六个雕文槽引用；详情表单单行四列等宽，未新增说明文本。实测 365/365 行通过物理值域校验，355 条效果 74 引用全部完整，法术图标无断链，角色库当前无非零雕文引用；3 条 `SpellID` 在现有法术镜像缺失，另有 5 条未被效果 74 引用的客户端残留行使用 `2/64/128/162`，均按既有数据保留。世界库 fallback `glyphproperties_dbc` 与 `glyphslot_dbc` 结构与 core 一致且当前为空。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 235 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `talent` | `foxy.dbc_talent` 23 列；详情页无关联 Tab；新增直接引用 `foxy.dbc_talent_tab` 24 列；关联 `foxy.dbc_spell.ID`、天赋自引用和角色库 `character_talent.spell` | Foxy `3b51e5959cab8de2181631c7ebd0cf781a820414`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `acore_characters` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/Rank 连续性/专属 Flags/Picker/标量管理/四列布局/引用保护及 DBC definition/本地化/字段/导出契约测试通过；892 条全量 Entity 校验及 Repository 分页、计数、新建编号、法术和组合筛选冒烟通过；`git diff --check` 通过 | 2026-07-16 | 主表 23 列和新增 DBC 24 列均为独立标量字段，9 个 Rank、3 组前置天赋/等级在 Entity、ViewModel、UI 中均显式管理；详情表单全部四列等宽，未新增说明文本。`TabID` 按 `TalentTab.dbc` 独立引用处理，9 个 Rank 和 `RequiredSpellID` 使用 Spell Picker，3 个前置天赋使用 Talent Picker；`Flags` 按 `TalentEntry::addToSpellBook` 与实测数据校准为本表专属 0/1，不复用相似枚举。AzerothCore 当前只消费前 5 个 Rank、第一组前置和 Flags；其余客户端物理列继续保留并校验，不删除或伪造语义。Repository 修复预分配 ID 和新建日志类型，校验 Rank 连续性、全部法术/天赋页/前置引用，并保护天赋自引用与可发现角色库引用。新增 required `TalentTab.dbc` 的全标量 Entity/Repository/Picker/本地化/导出闭环，现有用户需重新导入 DBC；实测库尚无该镜像表。实测 892/892 行通过校验，2358 个非零 Rank 法术引用全部完整，7 条角色天赋引用均可映射；11 条无前置天赋但等级非零、2 条缺失前置 ID 和 4 组宠物树重复坐标是 core 对应路径忽略的既有数据，允许原值不变但禁止新增同类断链。世界库 fallback `talent_dbc`、`talenttab_dbc` 当前均为空。Windows UI 手工点击未验证。全量 `flutter test` 为 246 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `currency_type` | `foxy.dbc_currency_types` 4 列；详情页无关联 Tab；新增直接引用 `foxy.dbc_currency_category` 19 列；关联 `item_template.entry` 和角色库 `characters.knownCurrencies` | Foxy `ea06fc4f6f4d3dbf8d280687277873453a075120`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `acore_characters` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/位索引/唯一性/Picker/标量管理/四列布局/引用保护及 DBC definition/本地化/字段/导出契约测试通过；26 条全量 Entity 校验及 Repository 分页、计数、名称筛选和新建编号冒烟通过；`git diff --check` 通过 | 2026-07-16 | 主表 4 列和新增 DBC 19 列均为独立标量字段；详情页单行四列等宽，未新增说明文本。`ItemID` 精确引用货币物品，`CategoryID` 精确引用独立 `CurrencyCategory.dbc`，不复用名称相似枚举；`BitIndex` 按 core 的 `PLAYER_FIELD_KNOWN_CURRENCIES` 64 位协议校验为唯一 1..64。Repository 校验 ItemID/BitIndex 唯一性、货币背包位、分类和可发现角色库位引用，并移除会制造重复核心索引的复制入口；修复 locale JOIN 未限定语言导致 26 行膨胀为 50 行以及名称筛选未生效。新增 required `CurrencyCategory.dbc` 全标量 Entity/Repository/Picker/本地化/导出闭环，现有用户需重新导入 DBC；实测库尚无该镜像表。实测 26/26 行通过物理值域校验，ItemID 与 BitIndex 均唯一且角色库当前无已用货币位；客户端既有记录中 CategoryID 24 和 2089878896 不在本地 CurrencyCategory DBC，ItemID 44209 不带货币背包位，均允许原值不变但禁止新增同类断链。世界库 fallback `currencytypes_dbc` 当前为空。Windows UI 手工点击未验证。全量 `flutter test` 为 257 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `scaling_stat_value` | `foxy.dbc_scaling_stat_values` 24 列；详情页无关联 Tab；关联 `item_template.ScalingStatValue` 位掩码和 `foxy.dbc_scaling_stat_distribution` | Foxy `6786e15deb0662e948a6f31cd3bfb82cdb2eb4ff`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/物理顺序/专属位掩码/值域/标量管理/四列布局/引用保护及 DBC definition/字段/导出契约测试通过；80 条全量 Entity 校验及 Repository 两页分页、计数、等级筛选、新建编号和导出顺序冒烟通过；`git diff --check` 通过 | 2026-07-16 | 24 个物理列均为独立标量字段，详情页 8 行全部四列等宽，未新增说明文本。AzerothCore 的 `ScalingStatValuesfmt` 首列为普通 `i` 而非索引 `n`，运行时按 DBC 物理记录序号查找；官方文件 ID 不连续且 70/80 行不等于 Charlevel，因此 Repository 改为按唯一连续 Charlevel 导出，新建/复制同步分配下一等级，禁止修改既有等级并保护共享等级记录删除。`item_template.ScalingStatValue` 按 `ScalingStatValuesEntry` 的实际分支重建为 22 个专属位：修正 `PrimaryBudget=0x8`、`RangedBudget=0x10`、`TertiaryBudget=0x40000`，补齐 `PlateChestArmor=0x800000`，排除恒返回 0 的 `0x10000` 和未消费的 `0x20000`，预算/护甲/DPS 各组最多选择一位。实测镜像 80/80 行与本地 DBC 逐列一致，Repository 顺序与文件顺序零差异；38 个物品使用 15 种掩码，未知位、未消费位、同组多选及 Distribution 断链均为 0。世界库 fallback `scalingstatvalues_dbc` 当前为空；未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 266 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `item_set` | `foxy.dbc_item_set` 53 列；详情页无关联 Tab；直接引用 `foxy.dbc_skill_line` 56 列、`foxy.dbc_spell.ID` 和 `item_template.entry`，关联 `item_template.itemset`、`item_set_names`、`item_set_names_locale` | Foxy `286fe910f7ede5d5d67b3c05847b531da01b6db1`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/物品槽/法术门槛/技能要求/精确 Picker/标量管理/四列布局/引用保护及 DBC definition/字段/导出契约共 15 项通过；509 条全量 Entity 校验、与本地 DBC 逐列比较及 Repository 分页、计数、单条回读、新建编号冒烟通过；`git diff --check` 通过 | 2026-07-16 | ItemSet 53 列和 SkillLine 56 列均改为一一对应的独立标量字段，移除 SkillLine Entity 的整行 Map 及 ItemSet ViewModel/UI 的索引分派和循环；详情页所有行四列等宽，8 组“法术+触发件数”成对展示，需求技能使用准确 SkillLine Picker，未新增说明文本。AzerothCore `ItemSetEntryfmt` 仅加载 `ItemID0..9` 并跳过客户端 `ItemID10..16`，实际后 7 槽均为 0；套装效果实际由 `item_template.itemset` 驱动，4,480 个非零反向引用全部有效。DBC 列表与运行时关系并非同一约束，实测存在 127 个列表关系差异和 2,126 个反向未列出物品，因此未擅自强制两边相等。实测 509/509 行与本地 ItemSet.dbc 的 53 列零差异，2,481 个物品和 1,153 个法术引用均完整，法术/门槛全部成对且门槛为 2..8；14 条技能要求均存在于本地 SkillLine.dbc。当前库尚未重新导入已注册的 required `dbc_skill_line`，故允许既有技能值原样保存，但新增或修改非零技能前要求先重新导入。`item_set_names` 2,481 行及 locale 14,850 行均符合 core 前 10 槽加载集合；世界库 fallback `itemset_dbc` 为 53 列且当前为空。Windows UI 手工点击未验证。全量 `flutter test` 为 273 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `achievement` | `foxy.dbc_achievement` 62 列；详情页无关联 Tab；新增直接依赖 `foxy.dbc_achievement_category` 20 列和 `foxy.dbc_achievement_criteria` 31 列；只读核验世界库 `achievement_reward` 8 列、`achievement_reward_locale` 4 列及角色完成/条件进度引用 | Foxy `5a3ab60548bc9d4dbad0ca8fbbddb0ac637ec40d`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；warcrafty / AzerothCore 3.3.5a DBC；实际 `acore_world` / `acore_characters` / `foxy` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | `flutter analyze` 通过；模块字段/专属阵营/Flags/稀疏 Criteria 类型/signed locale flags/精确 Picker/标量管理/四列布局/引用保护及 DBC definition/本地化/字段/导出契约 30 项通过；Repository 分页、计数、单条回读和新建编号冒烟通过；`git diff --check` 通过 | 2026-07-16 | Achievement 62 列、Category 20 列和 Criteria 31 列均为一一对应的独立标量字段。阵营使用本表专属 `-1/0/1`，地图、前置成就、分类、图标和共享条件分别使用准确 Picker，Flags 精确使用当前 core 的 10 个独立位；Criteria Type 使用 AzerothCore 稀疏枚举并保留实际客户端存在但 core 未命名的 118/120..123，不伪造枚举名称。`Minimum_criteria` 按共享条件来源的实际 Criteria 数量校验，删除保护覆盖成就自引用、Criteria、世界奖励和角色完成记录；客户端 `Reward_lang_*` 文案与世界表称号/物品/邮件奖励保持为不同关系。详情页全部四列等宽，未新增说明文本或 Tab。实测数据库 1,823 条成就全部通过校验，其中 1,817 条与本地 Achievement.dbc 62 列逐字段零差异，另有 6 条自定义记录；本地 86 条 Category 和 7,655 条 Criteria 全量 Entity 校验通过。世界奖励 115 条、奖励 locale 229 条和角色完成记录 12 条均无孤立引用。新增 required `Achievement_Category.dbc`、`Achievement_Criteria.dbc` 全标量 Entity/Repository/Picker/本地化/导出闭环，现有用户需重新导入 DBC；实测 `foxy` 库尚无这两张镜像表。现有 `dbc_spell_icon` 镜像缺少 83 个成就图标引用，客户端 Criteria 源文件有 196 条父成就在本地 Achievement 源文件中不存在，均属于既有来源/镜像问题，允许未改原值但禁止新增断链。Windows UI 手工点击未验证。全量 `flutter test` 为 280 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `reference_loot_template` | `reference_loot_template` 10 列；复合主键 `(Entry, Item)`；详情页无关联 Tab；按行类型关联 `item_template.entry` 或 `reference_loot_template.Entry` | Foxy `0a1aac82f81967037baefbeab19c6e2743882055`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；3.3.5a；实际 `acore_world` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | 10/10 字段已验证；`flutter analyze` 通过；模块字段/默认值/复合键/条件外键/LootMode Flags/跨字段约束/标量管理/四列布局契约 8 项及共享掉落回归共 38 项通过；Repository 行列表、分组 Picker、计数和单条回读冒烟通过；`git diff --check` 通过 | 2026-07-16 | `Chance` 校准为 SQL 默认 100；物品行使用 Item Picker，引用行保留 `Item` 的行标识语义，`Reference` 使用本表模板 Picker，写入前分别校验真实物品或 `abs(Reference)` 模板。`LootMode` 使用精确 Flags；除 `SharedDefines.h::LootModes` 六个位外，纳入 core base 中戈多克贡品第 5 档实际使用且 C++ 按位消费的 `0x20`，不借用其他相似枚举。引用行禁止服务端会忽略的任务标志，并要求 MinCount/MaxCount 一致；复制引用掉落行分配新 Entry，不再伪造 Item。详情表单三行均为四列等宽，未新增说明文本。实测结构与 core SQL 10 列一致，24,097/24,097 行满足加载约束，共 1,508 个模板、19,387 条物品行和 4,710 条引用行；物品与引用孤立数均为 0，未知 LootMode 为 0。修复 Picker 总数误按 24,097 行而非 1,508 个 Entry 计数。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 288 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
| `page_text` | `page_text` 4 列；主键 `ID`；`page_text_locale` 4 列、复合主键 `(ID, locale)`；关联 `NextPageID`、`item_template.PageText`、类型 9 `gameobject_template.Data0`、类型 10 `gameobject_template.Data7` | Foxy `7963c0d407a6d6919014020eadb2df48c7f9a884`；AzerothCore `master@a0b6553b5b83771e5a4a59bf3b68e76c6ce90826`；3.3.5a；实际 `acore_world` 库经 Database → Laconic → laconic_mysql 只读结构、全量数据及 Repository 检查 | **已完成** | 8/8 字段已验证；`flutter analyze` 通过；模块物理列/标量 Entity/数值范围/locale 精确枚举/链式引用/复合键/反向引用/四列布局/Picker 契约 9 项通过；Repository 分页、计数、单条回读和新建编号冒烟通过；`git diff --check` 通过 | 2026-07-16 | 完整 `PageTextEntity` 移除非物理列 `localeText`，列表本地化投影仅保留在 Brief Entity。`NextPageID` 使用页面文本 Picker，写入前校验目标存在并沿完整链阻止自引用及循环；删除保护覆盖下一页、物品、类型 9 文本对象和类型 10 Goober 四类反向引用，删除主记录时事务清理所属 locale。locale 精确采用 `Common.h::LocaleConstant` 中加载器实际接受的 8 个非 `enUS` 代码；每条记录明确编辑 `ID/locale/Text/VerifiedBuild`，拒绝重复复合键和孤儿写入，不再固定分配 16 个文本 Controller。主表与 locale 表单均为每行四列等宽，新建主记录保存前禁用 locale Tab，保存取得真实 ID 后启用；未新增说明文本。实测主表 1,951 行全部文本非空，`NextPageID` 缺失、自引用和循环均为 0；locale 11,079 行的代码全部合法，其中 28 条为既有孤儿；物品引用 398 条和 Goober 引用 46 条均无孤立，类型 9 文本对象引用 154 条中有 13 条既有孤立。Foxy 阻止新增断链但不擅自修复存量数据。未新增 required DBC。Windows UI 手工点击未验证。全量 `flutter test` 为 297 项通过、1 项跳过，仅剩任务前已可复现的 `dbc_sync_util_test.dart` 无效 MySQL 配置失败，与本模块无关。 |
