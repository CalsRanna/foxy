...
flutter test                             # ~112 test files; can take a while
...
- **Release flow** (trigger: pushing a `v*` tag): 1) manually bump `version` in `packages/foxy/pubspec.yaml` and add a `## vX.Y.Z` section to `CHANGELOG.md`, commit; 2) `git tag v1.x.y && git push origin v1.x.y`. The single job in `.github/workflows/release.yml` prepares the Windows environment once, then builds the Windows release, compiles the updater, zips, generates `latest.yaml` (SHA-256, keeps the 3 most recent releases, `appId: com.calsranna.foxy`), and publishes the GitHub Release. Recovery from a bad release: delete the tag and re-push; a `concurrency` group cancels stale runs. Analyze + tests (app, generator, lint) are not part of the release workflow; they run in `.github/workflows/ci.yml` on push and pull requests, on Windows to match the release environment. `latest.yaml` is the fixed update entry point.
...
- **Integration tests** (`test/integration/*_integration_test.dart`, tag `integration`): run the app's real startup path (`Database.instance.connect` → `MigrationRunner.run`) against a throwaway MySQL — database bootstrap, every migration, the utf8mb4 self-heal scan and a Chinese round-trip of the seeded `foxy.features` rows — plus repository read/write against a real server (generated `store`/`update`/`destroy`, their exception mapping, the hand-written list methods with their locale JOIN / `like` escaping / joined pagination, a reduced `creature_template` list fixture, the DBC import pipeline (staging table → row-count/duplicate-ID validation → atomic rename) plus an export→import round trip, and a `DatabaseTransaction` batch that must roll back on failure; fixture tables live in a dedicated `foxy_it_world` schema, while the DBC suite is gated behind `FOXY_TEST_MYSQL_ALLOW_DBC_WRITE=1` because it replaces the fixed `foxy.dbc_*` tables). Skipped by default (declared in `dart_test.yaml`), so a plain `flutter test` needs no database; CI runs them in a separate **Linux** job because GitHub-hosted Windows runners support neither service containers nor a preinstalled MySQL server. `MigrationRunner` hardcodes the `foxy` schema name (`create database if not exists foxy`), so these suites must stay serialized (`-j 1`) and must never be pointed at a database whose `foxy` schema matters.
