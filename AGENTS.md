...
| `@FoxyFilter.text/integer/decimal/boolean(name, defaultValue)` | Repository class | Generates the `<Name>Filter` query input object (with `fromJson`/`copyWith`/`toJson`) |
...
- Standard CRUD may not be handwritten: the generator rejects `get*/store*/update*/destroy*` methods that match the generated signatures. Specialized behavior goes into the overridable `_beforeStore`/`_beforeUpdate`/`_beforeDestroy` hooks or into separately named methods (`copy*`, `getBrief*`, `count*`).
...
`@FoxyListViewModel` reads the repository's `@FoxyFilter` annotations and generates: the `items`/`page`/`total`/`loading`/`submitting`/`errorMessage` signals, one `StringFieldController` per filter field, `search`/`reset`/`paginate` (each calls `markQueryVersion()`), `copy`/`destroy` (submitting guard, `normalizePageAfterDelete`, activity-log hook), `initSignals`, `dispose`, and `_refresh` with `_refreshToken` stale-response protection. Only `@FoxyFilter.text` filters are currently supported for lists. The repository's `getBrief*`/`count*`/`copy*` methods are matched by signature (a `Future<List<BriefXxxEntity>>` with a `filter` named parameter, a `Future<int>` with `filter`, a single `Future<Key>`/`Future<void>` taking `key`), not by name, so modules like `reference_loot_template` whose method names diverge from the base name still work. The shell overrides the empty `_logActivity` hook to record module-specific activity.
