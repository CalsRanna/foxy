...
Future<void> runDbcImportWorker(DbcImportWorkerArgs args) async {
  ...
  try {
    DbcImportWorker._throwIfCancelled(cancelled);
    workerStage = 'scanning';
    DbcImportWorker._sendStatus(sendPort, 'scanning', '正在扫描 DBC 目录...');
    final files = await DbcImportWorker._scanDirectory(directory);
    if (files.isEmpty) {
      errors.add(
    ...
    DbcImportWorker._sendCount(sendPort, '扫描完成', 0, files.length, 0, null);
    DbcImportWorker._throwIfCancelled(cancelled);
    ...
    for (var index = 0; index < files.length; index++) {
      ...
      if (targetExists) {
        try {
          compatible = await DbcImportWorker._tableMatchesSchema(
            connection,
            tableShort,
            file.fields,
          );
      ...
      try {
        ...
        if (targetExists && compatible) {
          ...
          await DbcImportWorker._createTable(connection, staging, file.fields);
        }
        stagingExists = true;
        ...
        await DbcImportWorker._ensureRowOrderColumn(connection, stagingShort);

        final importedRows = await connection.transaction(
          () => DbcImportWorker._importFile(
            connection,
            file,
            staging,
            sendPort,
        ...
        if (idRows.isEmpty ||
            DbcImportWorker._asInt(idRows.first['total_rows']) !=
                DbcImportWorker._asInt(idRows.first['distinct_ids'])) {
...
abstract final class DbcImportWorker {
  ...
  /// through.
  static String _uint64ToString(dynamic record, int index) {
  ...
  static String _readAndEscape(dynamic record, int index, String type) {
    return switch (type) {
  ...
  static Future<List<_FileDef>> _scanDirectory(String directory) async {
  ...
  static Future<bool> _tableMatchesSchema(
    Laconic connection,
    String tableShort,
    List<_FieldDef> fields,
  ) async {
  ...
  static void _throwIfCancelled(bool cancelled) {
    if (cancelled) throw const _ImportCancelled();
  }

  static _WorkerError _workerError({
    String? tableName,
    String? fileName,
