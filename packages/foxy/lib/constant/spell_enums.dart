// Spell enum constant definitions

abstract final class SpellEnums {
  /// Attribute type (MiscValue when Effect=6, Aura=29 MOD_STAT — note: this
  /// is the MOD_STAT-specific mapping, different from the Stats enum!)
  static const statTypeOptions = {
    -1: '全部属性',
    0: '法力值',
    1: '生命值',
    3: '敏捷',
    4: '力量',
    5: '智力',
    6: '精神',
    7: '耐力',
  };

  /// Stats enum (MiscValue when Effect=6, Aura=174/175 uses the AC Stats enum
  /// values directly)
  static const statsEnumOptions = {0: '力量', 1: '敏捷', 2: '耐力', 3: '智力', 4: '精神'};

  /// ReputationRank options (source: SharedDefines.h ReputationRank)
  static const spellReputationRankOptions = {
    0: '仇恨',
    1: '敌对',
    2: '冷淡',
    3: '中立',
    4: '友善',
    5: '尊敬',
    6: '崇敬',
    7: '崇拜',
  };

  /// SpellPreventionType options (source: SharedDefines.h enum
  /// SpellPreventionType)
  static const spellPreventionTypeOptions = {0: '无', 1: '沉默', 2: '平静'};

  /// Powers options (source: SharedDefines.h enum Powers)
  static const spellPowerTypeOptions = {
    0: '法力',
    1: '怒气',
    2: '集中值',
    3: '能量',
    4: '快乐值',
    5: '符文',
    6: '符能',
    127: '全部能量',
    0xFFFFFFFE: '生命值',
  };

  /// SpellModOp（MiscValue when Effect=6, Aura=107/108 ADD_FLAT/PCT_MODIFIER）
  static const spellModOpOptions = {
    0: '伤害',
    1: '持续时间',
    2: '威胁',
    3: '效果1',
    4: '次数',
    5: '范围',
    6: '半径',
    7: '暴击几率',
    8: '所有效果',
    9: '不退条',
    10: '施法时间',
    11: '冷却',
    12: '效果2',
    13: '忽略护甲',
    14: '消耗',
    15: '暴击伤害加成',
    16: '抵抗未命中几率',
    17: '跳跃目标数',
    18: '成功几率',
    19: '激活时间',
    20: '伤害倍率',
    21: '全局冷却',
    22: '持续伤害',
    23: '效果3',
    24: '加成倍率',
    26: '每分钟触发',
    27: '数值倍率',
    28: '驱散抵抗几率',
    30: '失败退还消耗',
  };

  /// Mechanics options (source: SharedDefines.h enum Mechanics)
  static const spellMechanicOptions = {
    0: '无',
    1: '魅惑',
    2: '迷惑',
    3: '缴械',
    4: '扰乱',
    5: '恐惧',
    6: '抓取',
    7: '定身',
    8: '减速攻击',
    9: '沉默',
    10: '睡眠',
    11: '减速',
    12: '昏迷',
    13: '冻结',
    14: '击倒',
    15: '流血',
    16: '绷带',
    17: '变形',
    18: '放逐',
    19: '护盾',
    20: '束缚',
    21: '坐骑',
    22: '感染',
    23: '转向',
    24: '惊骇',
    25: '无敌',
    26: '打断',
    27: '眩晕',
    28: '发现',
    29: '免疫护盾',
    30: '削弱',
    31: '激怒',
    32: '受伤',
    33: '感染2',
    34: '感染3',
    35: '感染4',
    36: '嘲讽',
  };

  /// SpellMgr.h SpellLinkedType; the database stores the index without
  /// multiplying by SPELL_LINKED_MAX_SPELLS.
  static const spellLinkedTypeOptions = {0: '施放', 1: '命中', 2: '光环'};

  /// ItemClass options (source: SharedDefines.h; -1 means unrestricted)
  static const spellItemClassOptions = {
    -1: '无限制',
    0: '消耗品',
    1: '容器',
    2: '武器',
    3: '宝石',
    4: '护甲',
    5: '试剂',
    6: '弹药',
    7: '商业物品',
    8: '通用',
    9: '配方',
    10: '货币',
    11: '箭袋',
    12: '任务物品',
    13: '钥匙',
    14: '永久物品',
    15: '杂项',
    16: '雕文',
  };

  /// Targets implicit-target options (source: SharedDefines.h enum Targets,
  /// 1-110)
  static const spellImplicitTargetOptions = {
    1: '施法者',
    2: '附近敌人',
    3: '附近友方',
    4: '附近队伍',
    5: '宠物',
    6: '目标敌人',
    7: '源区域条目',
    8: '目标区域条目',
    9: '目标家',
    11: '源区域未知11',
    15: '源区域敌人',
    16: '目标区域敌人',
    17: '目标DB',
    18: '目标点:施法者',
    20: '施法者区域队伍',
    21: '目标友方',
    22: '源:施法者',
    23: '游戏对象目标',
    24: '锥形敌人24',
    25: '目标任意',
    26: '游戏对象物品目标',
    27: '主人',
    28: '动态物体敌人',
    29: '动态物体友方',
    30: '源区域友方',
    31: '目标区域友方',
    32: '施法者召唤(左前方)',
    33: '源区域队伍',
    34: '目标区域队伍',
    35: '目标队伍',
    36: '目标点:施法者36',
    37: '最后目标区域队伍',
    38: '附近条目',
    39: '施法者钓鱼点',
    40: '附近游戏对象条目',
    41: '目标点:施法者右前方',
    42: '目标点:施法者右后方',
    43: '目标点:施法者左后方',
    44: '目标点:施法者左前方',
    45: '目标:链治疗友方',
    46: '附近目标条目',
    47: '目标点:施法者前方',
    48: '目标点:施法者后方',
    49: '目标点:施法者右侧',
    50: '目标点:施法者左侧',
    51: '游戏对象:源区域',
    52: '游戏对象:目标区域',
    53: '目标点:目标敌人',
    54: '锥形敌人54',
    55: '目标点:施法者跳跃前方',
    56: '施法者区域团队',
    57: '目标团队',
    58: '附近团队',
    59: '锥形友方',
    60: '锥形条目',
    61: '目标:区域团队职业',
    62: '未知62',
    63: '目标点:任意',
    64: '目标点:目标前方',
    65: '目标点:目标后方',
    66: '目标点:目标右侧',
    67: '目标点:目标左侧',
    68: '目标点:目标右前方',
    69: '目标点:目标右后方',
    70: '目标点:目标左后方',
    71: '目标点:目标左前方',
    72: '目标点:施法者周围随机',
    73: '目标点:施法者半径',
    74: '目标点:目标周围随机',
    75: '目标点:目标半径',
    76: '目标点:引导目标',
    77: '单位:引导目标',
    78: '目标点:目标点前方',
    79: '目标点:目标点后方',
    80: '目标点:目标点右侧',
    81: '目标点:目标点左侧',
    82: '目标点:目标点右前方',
    83: '目标点:目标点右后方',
    84: '目标点:目标点左后方',
    85: '目标点:目标点左前方',
    86: '目标点:目标点周围随机',
    87: '目标点',
    88: '动态物体(无)',
    89: '轨迹',
    90: '迷你宠物',
    91: '目标点:目标点半径',
    92: '召唤者',
    93: '尸体:源区域敌人',
    94: '载具',
    95: '目标:载具乘客',
    96: '乘客0',
    97: '乘客1',
    98: '乘客2',
    99: '乘客3',
    100: '乘客4',
    101: '乘客5',
    102: '乘客6',
    103: '乘客7',
    104: '锥形敌人104',
    105: '未知105',
    106: '目标点:引导施法者',
    107: '目标区域未知107',
    108: '游戏对象锥形',
    110: '目标未知110',
  };

  /// SpellFamilyNames options (source: SharedDefines.h enum SpellFamilyNames)
  static const spellFamilyNameOptions = {
    0: '通用',
    1: '事件/节日',
    3: '法师',
    4: '战士',
    5: '术士',
    6: '牧师',
    7: '德鲁伊',
    8: '盗贼',
    9: '猎人',
    10: '圣骑士',
    11: '萨满',
    12: '沉默抵抗',
    13: '药水',
    15: '死亡骑士',
    17: '宠物',
  };

  /// SpellEffects options (source: SharedDefines.h enum SpellEffects, 1-164)
  static const spellEffectOptions = {
    0: '无',
    1: '即死',
    2: '法术伤害',
    3: '占位效果',
    4: '传送门传送',
    5: '传送单位',
    6: '施加光环',
    7: '环境伤害',
    8: '能量吸取',
    9: '生命吸取',
    10: '治疗',
    11: '绑定',
    12: '传送门',
    13: '仪式基础',
    14: '仪式专精',
    15: '激活传送门',
    16: '完成任务',
    17: '武器伤害(无类型)',
    18: '复活',
    19: '额外攻击',
    20: '躲闪',
    21: '闪避',
    22: '招架',
    23: '格挡',
    24: '创建物品',
    25: '武器',
    26: '防御',
    27: '持久区域光环',
    28: '召唤',
    29: '跳跃',
    30: '充能',
    31: '武器百分比伤害',
    32: '触发飞弹',
    33: '开锁',
    34: '召唤替换物品',
    35: '队伍区域光环',
    36: '学习法术',
    37: '法术防御',
    38: '驱散',
    39: '语言',
    40: '双持',
    41: '跳到目标',
    42: '跳到目标点',
    43: '传送单位面朝施法者',
    44: '技能提升',
    45: '增加荣誉',
    46: '生成生物',
    47: '商业技能',
    48: '潜行',
    49: '侦测',
    50: '传送物体',
    51: '强制暴击',
    52: '保证命中',
    53: '附魔物品',
    54: '临时附魔物品',
    55: '驯服生物',
    56: '召唤宠物',
    57: '学习宠物技能',
    58: '武器伤害',
    59: '创建随机物品',
    60: '熟练度',
    61: '发送事件',
    62: '能量燃烧',
    63: '威胁',
    64: '触发法术',
    65: '团队区域光环',
    66: '创建法力宝石',
    67: '治疗最大生命值',
    68: '打断施法',
    69: '扰乱',
    70: '拉拽',
    71: '偷窃',
    72: '远视',
    73: '重置天赋',
    74: '应用雕文',
    75: '机械治疗',
    76: '召唤野外物体',
    77: '脚本效果',
    78: '攻击',
    79: '庇护所',
    80: '增加连击点',
    81: '创建房屋',
    82: '绑定视野',
    83: '决斗',
    84: '卡住',
    85: '召唤玩家',
    86: '激活物体',
    87: '游戏对象伤害',
    88: '游戏对象修复',
    89: '游戏对象设置破坏状态',
    90: '击杀奖励',
    91: '威胁全部',
    92: '附魔持有物品',
    93: '强制取消选择',
    94: '自我复活',
    95: '剥皮',
    96: '冲锋',
    97: '施法按钮',
    98: '击退',
    99: '分解',
    100: '醉酒',
    101: '喂食宠物',
    102: '解散宠物',
    103: '声望',
    104: '召唤物体槽1',
    105: '召唤物体槽2',
    106: '召唤物体槽3',
    107: '召唤物体槽4',
    108: '驱散机制',
    109: '复活宠物',
    110: '摧毁所有图腾',
    111: '耐久伤害',
    112: '未知112',
    113: '新复活',
    114: '攻击我',
    115: '耐久伤害百分比',
    116: '剥玩家尸体',
    117: '灵魂治疗',
    118: '技能',
    119: '宠物区域光环',
    120: '墓地传送',
    121: '标准化武器伤害',
    122: '未知122',
    123: '飞行路线',
    124: '拉向施法者',
    125: '修改威胁百分比',
    126: '偷取有益Buff',
    127: '选矿',
    128: '友方区域光环',
    129: '敌方区域光环',
    130: '重定向威胁',
    131: '播放声音',
    132: '播放音乐',
    133: '遗忘专精',
    134: '击杀奖励2',
    135: '呼唤宠物',
    136: '百分比治疗',
    137: '百分比充能',
    138: '后跳',
    139: '清除任务',
    140: '强制施法',
    141: '带值强制施法',
    142: '带值触发法术',
    143: '主人区域光环',
    144: '击退目标',
    145: '拉向目标',
    146: '激活符文',
    147: '任务失败',
    148: '带值触发飞弹',
    149: '冲锋目标',
    150: '任务开始',
    151: '触发法术2',
    152: '召唤RAF好友',
    153: '创建驯服宠物',
    154: '发现飞行点',
    155: '泰坦之握',
    156: '棱彩附魔物品',
    157: '创建物品2',
    158: '研磨',
    159: '允许重命名宠物',
    160: '强制施法2',
    161: '天赋专精数量',
    162: '天赋专精选择',
    163: '未知163',
    164: '移除光环',
  };

  /// SpellDmgClass options (source: SharedDefines.h enum SpellDmgClass)
  static const spellDmgClassOptions = {0: '无', 1: '魔法', 2: '近战', 3: '远程'};

  /// DispelType options (source: SharedDefines.h enum DispelType)
  static const spellDispelTypeOptions = {
    0: '无',
    1: '魔法',
    2: '诅咒',
    3: '疾病',
    4: '中毒',
    5: '潜行',
    6: '隐形',
    7: '全部',
    8: '特殊NPC',
    9: '激怒',
    10: 'ZG硬币',
  };

  /// AuraType options (source: SpellAuraDefines.h enum AuraType, 0-316)
  static const spellAuraTypeOptions = {
    0: '无',
    1: '绑定视野',
    2: '控制目标',
    3: '周期性伤害',
    4: '占位光环',
    5: '迷惑',
    6: '魅惑',
    7: '恐惧',
    8: '周期性治疗',
    9: '修改攻击速度',
    10: '修改威胁值',
    11: '嘲讽',
    12: '昏迷',
    13: '修改造成伤害',
    14: '修改受到伤害',
    15: '伤害护盾',
    16: '潜行',
    17: '侦测潜行',
    18: '隐形',
    19: '侦测隐形',
    20: '修改生命值(仅观察)',
    21: '修改能量(仅观察)',
    22: '修改抗性',
    23: '周期性触发法术',
    24: '周期性充能',
    25: '平静',
    26: '定身',
    27: '沉默',
    28: '反射法术',
    29: '修改属性',
    30: '修改技能',
    31: '增加移动速度',
    32: '增加坐骑速度',
    33: '降低移动速度',
    34: '增加生命值',
    35: '增加能量',
    36: '变形',
    37: '效果免疫',
    38: '状态免疫',
    39: '法术类型免疫',
    40: '伤害免疫',
    41: '驱散免疫',
    42: '触发法术',
    43: '触发伤害',
    44: '追踪生物',
    45: '追踪资源',
    46: '忽略装备测试法术',
    47: '修改招架百分比',
    48: '客户端周期性触发法术',
    49: '修改躲闪百分比',
    50: '修改暴击治疗加成',
    51: '修改格挡百分比',
    52: '修改武器暴击百分比',
    53: '周期性吸取',
    54: '修改命中几率',
    55: '修改法术命中几率',
    56: '变形',
    57: '修改法术暴击几率',
    58: '增加游泳速度',
    59: '修改对生物伤害',
    60: '平静+沉默',
    61: '修改缩放',
    62: '生命通道',
    63: '旧版周期性法力通道',
    64: '周期性法力吸取',
    65: '修改施法速度(不叠加)',
    66: '假死',
    67: '缴械',
    68: '追踪(被跟踪)',
    69: '法术吸收',
    70: '额外攻击',
    71: '修改某系法术暴击几率',
    72: '修改某系法力消耗百分比',
    73: '修改某系法力消耗',
    74: '反射某系法术',
    75: '修改语言',
    76: '远视',
    77: '机制免疫',
    78: '坐骑',
    79: '修改伤害百分比',
    80: '修改属性百分比',
    81: '分担伤害百分比',
    82: '水下呼吸',
    83: '修改基础抗性',
    84: '修改回复',
    85: '修改能量回复',
    86: '引导死亡物品',
    87: '修改受到伤害百分比',
    88: '修改生命回复百分比',
    89: '周期性伤害百分比',
    90: '旧版修改抵抗几率',
    91: '修改侦测范围',
    92: '阻止逃跑',
    93: '不可攻击',
    94: '打断回复',
    95: '幽灵',
    96: '法术磁铁',
    97: '法力护盾',
    98: '修改技能天赋',
    99: '修改攻击强度',
    100: '光环可见',
    101: '修改抗性百分比',
    102: '修改近战攻击强度对特定',
    103: '修改总威胁值',
    104: '水上行走',
    105: '缓落术',
    106: '悬停',
    107: '增加固定修正值',
    108: '增加百分比修正值',
    109: '触发时增加目标',
    110: '修改能量回复百分比',
    111: '施法者命中时触发',
    112: '覆盖职业脚本',
    113: '修改受到远程伤害',
    114: '修改受到远程伤害百分比',
    115: '修改治疗',
    116: '战斗中修改回复',
    117: '修改机制抗性',
    118: '修改治疗百分比',
    119: '旧版共享宠物追踪',
    120: '不可追踪',
    121: '共情',
    122: '修改副手伤害百分比',
    123: '修改目标抗性',
    124: '修改远程攻击强度',
    125: '修改受到近战伤害',
    126: '修改受到近战伤害百分比',
    127: '远程攻击强度攻击者加成',
    128: '控制宠物',
    129: '始终修改速度',
    130: '始终修改坐骑速度',
    131: '修改远程攻击强度对特定',
    132: '增加能量百分比',
    133: '增加生命值百分比',
    134: '修改法力回复打断',
    135: '修改治疗效果',
    136: '修改治疗效果百分比',
    137: '修改总属性百分比',
    138: '近战急速',
    139: '强制反应',
    140: '远程急速',
    141: '远程弹药急速',
    142: '修改基础抗性百分比',
    143: '修改独占抗性',
    144: '安全坠落',
    145: '修改宠物天赋点数',
    146: '允许驯服宠物类型',
    147: '机制免疫掩码',
    148: '保留连击点',
    149: '减少施法退条',
    150: '修改盾牌格挡值百分比',
    151: '追踪潜行',
    152: '修改被发现范围',
    153: '分担固定伤害',
    154: '潜行等级修正',
    155: '水下呼吸修正',
    156: '修改声望获取',
    157: '宠物伤害倍率',
    158: '修改盾牌格挡值',
    159: '无PVP奖励',
    160: '修改AOE回避',
    161: '战斗中修改生命回复',
    162: '能量燃烧',
    163: '修改暴击伤害加成',
    164: '未知164',
    165: '近战攻击强度攻击者加成',
    166: '修改攻击强度百分比',
    167: '修改远程攻击强度百分比',
    168: '修改对特定造成伤害',
    169: '修改对特定暴击百分比',
    170: '侦测护甲',
    171: '修改速度(不叠加)',
    172: '修改坐骑速度(不叠加)',
    173: '旧版允许冠军法术',
    174: '由属性百分比修改法术伤害',
    175: '由属性百分比修改法术治疗',
    176: '救赎之魂',
    177: 'AOE魅惑',
    178: '修改减益抗性',
    179: '修改攻击者法术暴击几率',
    180: '修改对特定固定法术伤害',
    181: '未知181',
    182: '由属性百分比修改抗性',
    183: '修改暴击威胁',
    184: '修改攻击者近战命中几率',
    185: '修改攻击者远程命中几率',
    186: '修改攻击者法术命中几率',
    187: '修改攻击者近战暴击几率',
    188: '修改攻击者远程暴击几率',
    189: '修改等级',
    190: '修改阵营声望获取',
    191: '使用正常移动速度',
    192: '近战远程急速',
    193: '近战减速',
    194: '修改目标某系吸收',
    195: '修改目标技能某系吸收',
    196: '修改冷却',
    197: '修改攻击者法术和武器暴击几率',
    198: '旧版修改所有武器技能',
    199: '增加法术命中百分比',
    200: '修改经验获取百分比',
    201: '飞行',
    202: '忽略战斗结果',
    203: '修改攻击者近战暴击伤害',
    204: '修改攻击者远程暴击伤害',
    205: '修改受到某系暴击伤害',
    206: '增加飞行速度',
    207: '增加坐骑飞行速度',
    208: '始终修改飞行速度',
    209: '始终修改坐骑飞行速度',
    210: '修改飞行速度(不叠加)',
    211: '修改坐骑飞行速度(不叠加)',
    212: '由属性百分比修改远程攻击强度',
    213: '由伤害获得的怒气',
    214: '未知214',
    215: '竞技场准备',
    216: '法术急速',
    217: '近战急速2',
    218: '远程急速',
    219: '由属性修改法力回复',
    220: '由属性修改等级',
    221: '减少威胁',
    222: '未知222',
    223: '团队充能触发',
    224: '未知224',
    225: '团队充能带值触发',
    226: '周期性占位',
    227: '周期性带值触发法术',
    228: '侦测潜行',
    229: '修改AOE伤害回避',
    230: '未知230',
    231: '带值触发法术',
    232: '修改机制持续时间',
    233: '所有人形模型变更为',
    234: '修改机制持续时间(不叠加)',
    235: '修改驱散抗性',
    236: '控制载具',
    237: '由攻击强度修改法术伤害',
    238: '由攻击强度修改法术治疗',
    239: '修改缩放2',
    240: '修改精准',
    241: '强制向前移动',
    242: '由法术治疗修改法术伤害',
    243: '修改阵营',
    244: '理解语言',
    245: '由驱散修改光环持续时间',
    246: '由驱散修改光环持续时间(不叠加)',
    247: '复制施法者',
    248: '修改战斗结果几率',
    249: '转换符文',
    250: '增加生命值2',
    251: '修改敌人躲闪',
    252: '所有速度减速',
    253: '修改格挡暴击几率',
    254: '缴械副手',
    255: '修改受到机制伤害百分比',
    256: '无需施法材料',
    257: '由法术类别修改目标抗性',
    258: '未知258',
    259: '修改HOT百分比',
    260: '屏幕效果',
    261: '相位',
    262: '技能忽略光环状态',
    263: '仅允许技能',
    264: '未知264',
    265: '未知265',
    266: '未知266',
    267: '免疫光环应用(某系)',
    268: '由属性百分比修改攻击强度',
    269: '忽略目标抗性修正',
    270: '技能忽略目标抗性',
    271: '由施法者修改伤害',
    272: '忽略近战重置',
    273: 'X射线',
    274: '技能不消耗弹药',
    275: '忽略变形',
    276: '对机制修改造成伤害',
    277: '修改最大受影响目标',
    278: '缴械远程',
    279: '初始化镜像',
    280: '护甲穿透百分比',
    281: '荣誉获取百分比',
    282: '基础生命值百分比',
    283: '修改受到治疗',
    284: '链接光环',
    285: '由护甲修改攻击强度',
    286: '技能周期性暴击',
    287: '偏转法术',
    288: '忽略命中方向',
    289: '防止耐久损失',
    290: '修改暴击百分比',
    291: '任务经验获取百分比',
    292: '打开兽栏',
    293: '覆盖法术',
    294: '防止能量回复',
    295: '未知295',
    296: '设置载具ID',
    297: '屏蔽法术族',
    298: '扼喉',
    299: '未知299',
    300: '分担伤害百分比',
    301: '某系治疗吸收',
    302: '未知302',
    303: '对光环状态修改造成伤害',
    304: '假醉酒',
    305: '修改最小速度',
    306: '未知306',
    307: '治疗吸收测试',
    308: '修改施法者暴击几率',
    309: '未知309',
    310: '修改生物AOE伤害回避',
    311: '未知311',
    312: '未知312',
    313: '未知313',
    314: '防止复活',
    315: '水下行走',
    316: '周期性急速',
  };

  /// SharedDefines.h Gender; spell_area.gender only accepts 0, 1, 2.
  static const spellAreaGenderOptions = {0: '男性', 1: '女性', 2: '不限'};

  /// Speed type (MiscValue when Effect=6, Aura=31 MOD_INCREASE_SPEED)
  static const speedTypeOptions = {0: '步行', 1: '跑步', 2: '游泳', 3: '飞行'};

  /// Shapeshift form (MiscValue when Effect=6, Aura=36 MOD_SHAPESHIFT)
  static const shapeshiftFormOptions = {
    0: '人形态',
    1: '熊形态',
    2: '水栖形态',
    3: '猫形态',
    4: '旅行形态',
    5: '枭兽形态/食尸鬼',
    6: '飞行形态',
    7: '树人形态',
    8: '迅捷飞行形态',
    31: '树人形态(带技能)',
  };

  /// Power type (MiscValue when Effect=30 ENERGIZE)
  /// Reuses SpellEnums.spellPowerTypeOptions, plus:
  static const energizePowerTypeOptions = {
    0: '法力',
    1: '怒气',
    2: '集中值',
    3: '能量',
    4: '快乐值',
    5: '符文',
    6: '符能',
  };

  /// AuraStateType (caster/target state)
  static const auraStateTypeOptions = {
    0: '无',
    1: '防御姿态',
    2: '生命低于20%',
    3: '狂暴',
    4: '冻结',
    5: '审判',
    7: '猎人招架',
    8: '被放逐',
    10: '乘胜追击',
    12: '精灵之火',
    13: '生命低于35%',
    14: '燃烧',
    15: '迅捷治愈',
    16: '致命毒药',
    17: '激怒',
    18: '流血',
    22: '未知22',
    23: '生命高于75%',
  };

  /// Attack-power type (MiscValue when Effect=6, Aura=99 MOD_ATTACK_POWER)
  static const attackPowerTypeOptions = {1: '远程攻击强度', 2: '近战攻击强度'};
}

// Source: AzerothCore SharedDefines.h, SpellDefines.h, SpellAuraDefines.h
