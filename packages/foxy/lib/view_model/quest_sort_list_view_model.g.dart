...
part of 'quest_sort_list_view_model.dart';

mixin _QuestSortListViewModelMixin on FieldControllerMixin, QueryVersionMixin {
  final _repository = GetIt.instance.get<QuestSortRepository>();

  final items = signal(<BriefQuestSortEntity>[]);
  ...
  Future<void> destroy(int key) async {
    ...
    errorMessage.value = null;
    try {
      final record = await _repository.getQuestSort(key);
  ...
  QuestSortFilter _collectFilter() {
  ...
  Future<void> _logActivity(
    ...
    int key, [
    QuestSortEntity? record,
  ]) async {
    final resolved = record ?? await _repository.getQuestSort(key);
    final entityName = resolved == null
        ? key.toString()
        : resolved.sortNameLangZhCN.isNotEmpty
        ? resolved.sortNameLangZhCN
        : key.toString();
    GetIt.instance.get<EventBus>().fire(
      EntityWrittenEvent(
        ActivityLogEntity(
          module: 'dbc_quest_sort',
          actionType: action,
  ...
  Future<void> _refresh() async {
    ...
    try {
      final (nextItems, nextTotal) = await (
        _repository.getBriefQuestSorts(page: currentPage, filter: filter),
