...
part of 'game_object_quest_ender_linked_list_view_model.dart';

mixin _GameObjectQuestEnderLinkedListViewModelMixin on FieldControllerMixin {
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createGameObjectQuestEnder(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> persist() async {
    ...
    try {
      ...
      try {
        _logActivity(
          action,
          originalKey ?? GameObjectQuestEnderKey.fromEntity(candidate),
        );
      } catch (_) {
        // Activity log is best-effort; failure (e.g. not registered in
        // tests) must not affect the main flow.
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(GameObjectQuestEnderEntity(quest: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countGameObjectQuestEnders(link);
