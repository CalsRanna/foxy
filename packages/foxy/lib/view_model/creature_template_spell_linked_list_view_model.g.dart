...
part of 'creature_template_spell_linked_list_view_model.dart';

mixin _CreatureTemplateSpellLinkedListViewModelMixin on FieldControllerMixin {
  final _repository = GetIt.instance.get<CreatureTemplateSpellRepository>();

  final linkKey = signal<int?>(null);

  final items = signal(<BriefCreatureTemplateSpellEntity>[]);

  final editingKey = signal<CreatureTemplateSpellKey?>(null);

  final selectedKey = signal<CreatureTemplateSpellKey?>(null);
  ...
  Future<void> copy(CreatureTemplateSpellKey key) async {
    ...
    try {
      await _repository.copyCreatureTemplateSpell(key);
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createCreatureTemplateSpell(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> destroy(CreatureTemplateSpellKey key) async {
    if (submitting.value) throw BusyException('operation already in progress');
    final link = linkKey.value;
    if (link == null) {
      throw LinkNotLoadedException('link record not loaded');
    ...
    errorMessage.value = null;
    try {
      await _repository.destroyCreatureTemplateSpell(key);
      if (token != _interactionToken || linkKey.value != link) return;
      try {
  ...
  Future<void> edit(CreatureTemplateSpellKey key) async {
    if (submitting.value) throw BusyException('operation already in progress');
    final link = linkKey.value;
    if (link == null) {
      throw LinkNotLoadedException('link record not loaded');
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.getCreatureTemplateSpell(key);
      if (token != _interactionToken || linkKey.value != link) return;
      if (candidate == null) {
  ...
  Future<void> persist() async {
    ...
    try {
      ...
      try {
        _logActivity(
          action,
          originalKey ?? CreatureTemplateSpellKey.fromEntity(candidate),
        );
      } catch (_) {
        // Activity log is best-effort; failure (e.g. not registered in
        // tests) must not affect the main flow.
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(CreatureTemplateSpellEntity(creatureID: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countCreatureTemplateSpells(link);
