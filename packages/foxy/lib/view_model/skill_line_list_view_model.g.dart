...
part of 'skill_line_list_view_model.dart';

mixin _SkillLineListViewModelMixin on FieldControllerMixin, QueryVersionMixin {
  final _repository = GetIt.instance.get<SkillLineRepository>();

  final items = signal(<BriefSkillLineEntity>[]);
  ...
  Future<void> copy(int key) async {
    ...
    try {
      await _repository.copySkillLine(key);
  ...
  Future<void> destroy(int key) async {
    ...
    errorMessage.value = null;
    try {
      final record = await _repository.getSkillLine(key);
      await _repository.destroySkillLine(key);
  ...
  SkillLineFilter _collectFilter() {
  ...
  Future<void> _logActivity(
    ...
    int key, [
    SkillLineEntity? record,
  ]) async {
    final resolved = record ?? await _repository.getSkillLine(key);
    final entityName = resolved == null
        ? key.toString()
        : resolved.displayNameLangZhCN.isNotEmpty
        ? resolved.displayNameLangZhCN
        : key.toString();
    GetIt.instance.get<EventBus>().fire(
      EntityWrittenEvent(
        ActivityLogEntity(
          module: 'dbc_skill_line',
          actionType: action,
  ...
  Future<void> _refresh() async {
    ...
    try {
      final (nextItems, nextTotal) = await (
        _repository.getBriefSkillLines(page: currentPage, filter: filter),
        _repository.countSkillLines(filter: filter),
      ).wait;
