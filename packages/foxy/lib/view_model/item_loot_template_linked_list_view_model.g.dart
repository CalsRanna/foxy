...
part of 'item_loot_template_linked_list_view_model.dart';

mixin _ItemLootTemplateLinkedListViewModelMixin on FieldControllerMixin {
  final _repository = GetIt.instance.get<ItemLootTemplateRepository>();

  final linkKey = signal<int?>(null);

  final items = signal(<BriefItemLootTemplateEntity>[]);

  final editingKey = signal<ItemLootTemplateKey?>(null);

  final selectedKey = signal<ItemLootTemplateKey?>(null);
  ...
  Future<void> copy(ItemLootTemplateKey key) async {
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createItemLootTemplate(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> destroy(ItemLootTemplateKey key) async {
    if (submitting.value) throw BusyException('operation already in progress');
    final link = linkKey.value;
    if (link == null) {
      throw LinkNotLoadedException('link record not loaded');
  ...
  Future<void> edit(ItemLootTemplateKey key) async {
    if (submitting.value) throw BusyException('operation already in progress');
    final link = linkKey.value;
    if (link == null) {
      throw LinkNotLoadedException('link record not loaded');
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.getItemLootTemplate(key);
      if (token != _interactionToken || linkKey.value != link) return;
      if (candidate == null) {
  ...
  Future<void> persist() async {
    ...
    try {
      if (originalKey == null) {
        await _repository.storeItemLootTemplate(candidate);
      } else {
        await _repository.updateItemLootTemplate(originalKey, candidate);
      }
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(ItemLootTemplateEntity(entry: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countItemLootTemplates(link);
      if (token != _refreshToken) return;
      final lastPage = max(1, (count / _repository.kPageSize).ceil());
      final nextPage = min(currentPage, lastPage);
      final data = await _repository.getBriefItemLootTemplates(
        link,
        page: nextPage,
      );
      if (token != _refreshToken) return;
