...
part of 'spell_list_view_model.dart';

mixin _SpellListViewModelMixin on FieldControllerMixin, QueryVersionMixin {
  final _repository = GetIt.instance.get<SpellRepository>();

  final items = signal(<BriefSpellEntity>[]);
  ...
  Future<void> destroy(int key) async {
    ...
    errorMessage.value = null;
    try {
      final record = await _repository.getSpell(key);
  ...
  Future<void> _logActivity(
    ...
    final resolved = record ?? await _repository.getSpell(key);
  ...
  Future<void> _refresh() async {
    ...
    try {
      final (nextItems, nextTotal) = await (
        _repository.getBriefSpells(page: currentPage, filter: filter),
