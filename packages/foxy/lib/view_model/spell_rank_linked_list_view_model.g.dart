...
part of 'spell_rank_linked_list_view_model.dart';

mixin _SpellRankLinkedListViewModelMixin on FieldControllerMixin {
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createSpellRank(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> persist() async {
    ...
    try {
      ...
      try {
        _logActivity(action, originalKey ?? SpellRankKey.fromEntity(candidate));
      } catch (_) {
        // Activity log is best-effort; failure (e.g. not registered in
        // tests) must not affect the main flow.
      }
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(SpellRankEntity(firstSpellId: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countSpellRanks(link);
      if (token != _refreshToken) return;
      final lastPage = max(1, (count / _repository.kPageSize).ceil());
      final nextPage = min(currentPage, lastPage);
      final data = await _repository.getBriefSpellRanks(link, page: nextPage);
      if (token != _refreshToken) return;
      page.value = nextPage;
      items.value = data;
      total.value = count;
