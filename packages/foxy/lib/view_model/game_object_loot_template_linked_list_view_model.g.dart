...
part of 'game_object_loot_template_linked_list_view_model.dart';

mixin _GameObjectLootTemplateLinkedListViewModelMixin on FieldControllerMixin {
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createGameObjectLootTemplate(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> destroy(GameObjectLootTemplateKey key) async {
    ...
    errorMessage.value = null;
    try {
      final record = await _repository.getGameObjectLootTemplate(key);
  ...
  Future<void> persist() async {
    ...
    try {
      ...
      try {
        await _logActivity(
          action,
          originalKey ?? GameObjectLootTemplateKey.fromEntity(candidate),
        );
      } catch (_) {
        // Activity log is best-effort; failure (e.g. not registered in
        // tests) must not affect the main flow.
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(GameObjectLootTemplateEntity(entry: link));
    await _refresh();
  }
  ...
  Future<void> _logActivity(
    ...
    GameObjectLootTemplateEntity? record,
  ]) async {
    final resolved = record ?? await _repository.getGameObjectLootTemplate(key);
    ...
    GetIt.instance.get<EventBus>().fire(
      EntityWrittenEvent(
        ActivityLogEntity(
          module: 'gameobject_loot_template',
          actionType: action,
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countGameObjectLootTemplates(link);
