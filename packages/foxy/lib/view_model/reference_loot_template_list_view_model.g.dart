...
part of 'reference_loot_template_list_view_model.dart';

mixin _ReferenceLootTemplateListViewModelMixin
    on FieldControllerMixin, QueryVersionMixin {
  final _repository = GetIt.instance.get<ReferenceLootTemplateRepository>();

  final items = signal(<BriefReferenceLootTemplateEntity>[]);
  ...
  Future<void> destroy(ReferenceLootTemplateKey key) async {
    ...
    errorMessage.value = null;
    try {
      final record = await _repository.getReferenceLootTemplate(key);
  ...
  Future<void> _logActivity(
    ...
    ReferenceLootTemplateEntity? record,
  ]) async {
    final resolved = record ?? await _repository.getReferenceLootTemplate(key);
