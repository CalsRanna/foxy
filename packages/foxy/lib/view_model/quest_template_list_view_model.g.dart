...
part of 'quest_template_list_view_model.dart';

mixin _QuestTemplateListViewModelMixin
    on FieldControllerMixin, QueryVersionMixin {
  final _repository = GetIt.instance.get<QuestTemplateRepository>();

  final items = signal(<BriefQuestTemplateEntity>[]);
  ...
  Future<void> destroy(int key) async {
    ...
    errorMessage.value = null;
    try {
      final record = await _repository.getQuestTemplate(key);
  ...
  Future<void> _logActivity(
    ...
    int key, [
    QuestTemplateEntity? record,
  ]) async {
    final resolved = record ?? await _repository.getQuestTemplate(key);
    final entityName = resolved == null
        ? key.toString()
        : resolved.logTitle.isNotEmpty
        ? resolved.logTitle
        : key.toString();
  ...
  Future<void> _refresh() async {
    ...
    try {
      final (nextItems, nextTotal) = await (
        _repository.getBriefQuestTemplates(page: currentPage, filter: filter),
