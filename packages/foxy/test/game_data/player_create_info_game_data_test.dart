...
void main() {
  test('种族职业 ID 与掩码分别对应 SharedDefines', () {
    expect(PlayerCreateInfoConstants.playerRaceOptions.keys.toList(), [
      1,
      2,
      3,
      4,
    ...
    expect(PlayerCreateInfoConstants.playerClassOptions.keys.toList(), [
      1,
      2,
      3,
      4,
  ...
  test('动作类型只包含 Player ActionButtonType', () {
    expect(
      PlayerCreateInfoConstants.playerActionButtonTypeOptions.keys.toSet(),
      {0, 1, 32, 64, 65, 128},
    );
  });
  ...
  test('种族/职业 ID 映射为标签，未知值回退', () {
    expect(
      const BriefPlayerCreateInfoEntity(race: 1, class_: 2).raceLabel,
      '人类',
    );
    expect(
      const BriefPlayerCreateInfoEntity(race: 1, class_: 2).classLabel,
      '圣骑士',
    );
  ...
  test('种族/职业掩码展开为标签，未命中回退', () {
    ...
    expect(
      const BriefPlayerCreateInfoSkillEntity(raceMask: 0).raceMaskLabel,
      '0',
    );
  });
}
