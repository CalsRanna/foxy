...
ipcMain.on(UPDATE_GAME_OBJECT_TEMPLATE, (event, payload) => {
  let queryBuilder = knex().table("gameobject_template").where("entry", payload.entry).update(payload);
...
ipcMain.on(CREATE_GAME_OBJECT_TEMPLATE, (event, payload) => {
  let queryBuilder = knex().select("entry").from("gameobject_template").orderBy("entry", "desc");
...
ipcMain.on(COPY_GAME_OBJECT_TEMPLATE, (event, payload) => {
  ...
  let entryQueryBuilder = knex().select("entry").from("gameobject_template").orderBy("entry", "desc");
  let findGameObjectTemplateQueryBuilder = knex().select().from("gameobject_template").where(payload);
  Promise.all([
    ...
    let queryBuilder = knex().insert(gameObjectTemplate).into("gameobject_template");
    queryBuilder.then((rows) => {
