...
ipcMain.on(UPDATE_QUEST_TEMPLATE, (event, payload) => {
  let queryBuilder = knex().table("quest_template").where("ID", payload.ID).update(payload);
...
ipcMain.on(CREATE_QUEST_TEMPLATE, (event, payload) => {
  let queryBuilder = knex().select("ID").from("quest_template").orderBy("ID", "desc");
...
ipcMain.on(COPY_QUEST_TEMPLATE, (event, payload) => {
  ...
  let idQueryBuilder = knex().select("ID").from("quest_template").orderBy("ID", "desc");
  let findQuestTemplateQueryBuilder = knex().select().from("quest_template").where(payload);
  Promise.all([
...
ipcMain.on(SEARCH_QUEST_TEMPLATE_LOCALES, (event, payload) => {
  let queryBuilder = knex().select().from("quest_template_locale").where("ID", payload.id);
