...
ipcMain.on(SEARCH_ITEM_SETS_FOR_SELECTOR, (event, payload) => {
  ...
  queryBuilder
    .then((rows) => {
      event.reply(SEARCH_ITEM_SETS_FOR_SELECTOR, rows);
    })
    .catch((error) => {
      event.reply(`${SEARCH_ITEM_SETS_FOR_SELECTOR}_REJECT`, error);
...
ipcMain.on(COUNT_ITEM_SETS_FOR_SELECTOR, (event, payload) => {
  ...
  queryBuilder
    .then((rows) => {
      event.reply(COUNT_ITEM_SETS_FOR_SELECTOR, rows[0].total);
    })
    .catch((error) => {
      event.reply(`${COUNT_ITEM_SETS_FOR_SELECTOR}_REJECT`, error);
      event.reply(GLOBAL_MESSAGE_BOX, JSON.stringify(error));
    })
    .finally(() => {
      event.reply(GLOBAL_MESSAGE, queryBuilder.toString());
