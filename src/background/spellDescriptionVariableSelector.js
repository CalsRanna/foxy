...
ipcMain.on(
  ...
  (event, payload) => {
    ...
    queryBuilder = queryBuilder
      .limit(payload.size)
      .offset(
        payload.page != undefined ? (payload.page - 1) * payload.size : 0
      );

    queryBuilder
      .then((rows) => {
        event.reply(SEARCH_SPELL_DESCRIPTION_VARIABLES_FOR_SELECTOR, rows);
      })
      .catch((error) => {
        event.reply(
...
ipcMain.on(COUNT_SPELL_DESCRIPTION_VARIABLES_FOR_SELECTOR, (event, payload) => {
  let queryBuilder = knex
    .count("* as total")
    .from("foxy.dbc_spell_description_variables");
  if (payload.ID) {
