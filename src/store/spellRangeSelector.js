const ipcRenderer = window.ipcRenderer;

import {
  SEARCH_SPELL_RANGES_FOR_SELECTOR,
  COUNT_SPELL_RANGES_FOR_SELECTOR,
  PAGINATE_SPELL_RANGES_FOR_SELECTOR,
} from "../constants";

export default {
  namespaced: true,
  state: () => ({
    pagination: {
      total: 0,
      page: 1,
      size: 50,
    },
    spellRanges: [],
  }),
  actions: {
    searchSpellRangesForSelector({ commit }, payload) {
      return new Promise((resolve, reject) => {
        ipcRenderer.send(SEARCH_SPELL_RANGES_FOR_SELECTOR, payload);
        ipcRenderer.once(
          SEARCH_SPELL_RANGES_FOR_SELECTOR,
          (event, response) => {
            commit(SEARCH_SPELL_RANGES_FOR_SELECTOR, response);
            resolve();
          }
        );
        ipcRenderer.once(
          `${SEARCH_SPELL_RANGES_FOR_SELECTOR}_REJECT`,
          (event, error) => {
            reject(error);
          }
        );
      });
    },
    countSpellRangesForSelector({ commit }, payload) {
      return new Promise((resolve, reject) => {
        ipcRenderer.send(COUNT_SPELL_RANGES_FOR_SELECTOR, payload);
        ipcRenderer.once(COUNT_SPELL_RANGES_FOR_SELECTOR, (event, response) => {
          commit(COUNT_SPELL_RANGES_FOR_SELECTOR, response);
          resolve();
        });
        ipcRenderer.once(
          `${COUNT_SPELL_RANGES_FOR_SELECTOR}_REJECT`,
          (event, error) => {
            reject(error);
          }
        );
      });
    },
    paginateSpellRangesForSelector({ commit }, payload) {
      return new Promise((resolve) => {
        commit(PAGINATE_SPELL_RANGES_FOR_SELECTOR, payload.page);
        resolve();
      });
    },
  },
  mutations: {
    [SEARCH_SPELL_RANGES_FOR_SELECTOR](state, spellRanges) {
      state.spellRanges = spellRanges;
    },
    [COUNT_SPELL_RANGES_FOR_SELECTOR](state, total) {
      state.pagination.total = total;
    },
    [PAGINATE_SPELL_RANGES_FOR_SELECTOR](state, page) {
      state.pagination.page = page;
    },
  },
};
