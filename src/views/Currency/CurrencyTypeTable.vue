<template>
  <div>
    <el-card>
      ...
      <h3 style="margin: 16px 0 0 0">货币列表</h3>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    async search() {
      this.loading = true;
      this.paginateCurrencyTypes({ page: 1 });
      await Promise.all([
        this.searchCurrencyTypes(this.payload),
        this.countCurrencyTypes(this.payload),
      ]);
      this.loading = false;
    },
    async reset() {
      ...
      this.$confirm("此操作不会复制关联表数据，确认继续？", "确认复制", {
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            this.copyCurrencyType({ ID: this.currentRow.ID })
              .then(() => {
                Promise.all([
                  this.searchCurrencyTypes(this.payload),
                  this.countCurrencyTypes(this.payload),
      ...
      this.$confirm(
        ...
        "确认删除",
        {
          ...
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              instance.confirmButtonLoading = true;
              this.destroyCurrencyType({ ID: this.currentRow.ID })
                .then(() => {
                  Promise.all([
                    this.searchCurrencyTypes(this.payload),
                    this.countCurrencyTypes(this.payload),
    ...
    async paginate(page) {
      this.loading = true;
      this.paginateCurrencyTypes({ page: page });
      await this.searchCurrencyTypes(this.payload);
      this.loading = false;
    },
    show(row) {
      this.$router.push(`/currency/${row.ID}`);
    },
    async init() {
      this.loading = true;
      await Promise.all([
