<template>
  <div>
    ...
    >
      ...
      <el-table :data="pageTextLocales" :max-height="calculateMaxHeight()">
        ...
        <el-table-column label="名称">
          <template slot-scope="scope">
            <el-input v-model="scope.row.Text" placeholder="Text"></el-input>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...mapActions("pageTextLocale", ["storePageTextLocales"]),
    calculateMaxHeight() {
    ...
    create() {
      this.pageTextLocales.push({
        ID: this.pageText.ID,
        VerifiedBuild: 0,
      });
    },
    destroy(index) {
      this.pageTextLocales.splice(index, 1);
    },
    async store() {
      this.loading = true;
      await this.storePageTextLocales(this.pageTextLocales);
      this.$notify({
        title: "保存成功",
