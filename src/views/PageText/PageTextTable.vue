  ...
  <div>
    <el-card>
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>页面文本管理</el-breadcrumb-item>
      </el-breadcrumb>
      <h3 style="margin: 16px 0 0 0">页面文本列表</h3>
    </el-card>
...
export default {
  ...
  methods: {
    ...
    async search() {
      this.loading = true;
      this.paginatePageTexts({ page: 1 });
      await Promise.all([
        this.searchPageTexts(this.payload),
        this.countPageTexts(this.payload),
      ]);
      this.loading = false;
    },
    async reset() {
    ...
    create() {
      this.$router.push("/page-text/create");
    },
    copy() {
      this.$confirm("此操作不会复制关联表数据，确认继续？", "确认复制", {
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            this.copyPageText({ ID: this.currentRow.ID })
              .then(() => {
                Promise.all([
                  this.searchPageTexts(this.payload),
                  this.countPageTexts(this.payload),
      ...
      this.$confirm(
        ...
        "确认删除",
        {
          ...
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              instance.confirmButtonLoading = true;
              this.destroyPageText({ ID: this.currentRow.ID })
                .then(() => {
                  Promise.all([
                    this.searchPageTexts(this.payload),
                    this.countPageTexts(this.payload),
    ...
    async paginate(page) {
      this.loading = true;
      this.paginatePageTexts({ page: page });
      await this.searchPageTexts(this.payload);
      this.loading = false;
    },
    show(row) {
      this.$router.push(`/page-text/${row.ID}`);
    },
    async init() {
      this.loading = true;
      await Promise.all([
