  ...
  <div>
    <el-card>
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item>附魔技能管理</el-breadcrumb-item>
      </el-breadcrumb>
      <h3 style="margin: 16px 0 0 0">附魔技能列表</h3>
    </el-card>
    ...
    <el-card style="margin-top: 16px">
      ...
      >
        ...
        <el-table-column prop="Charges" label="充能"></el-table-column>
        <el-table-column prop="Effect_1" label="效果"></el-table-column>
...
export default {
  ...
  methods: {
    ...mapActions("spellItemEnchantment", [
      "searchSpellItemEnchantments",
      "countSpellItemEnchantments",
      "paginateSpellItemEnchantments",
      "copySpellItemEnchantment",
      "destroySpellItemEnchantment",
      "resetCredential",
    ]),
    calculateMaxHeight() {
    ...
    async search() {
      this.loading = true;
      this.paginateSpellItemEnchantments({ page: 1 });
      await Promise.all([
        this.searchSpellItemEnchantments(this.payload),
        this.countSpellItemEnchantments(this.payload),
      ]);
      this.loading = false;
    },
    async reset() {
    ...
    create() {
      this.$router.push("/spell-item-enchantment/create");
    },
    copy() {
      this.$confirm("此操作不会复制关联表数据，确认继续？", "确认复制", {
        ...
        beforeClose: (action, instance, done) => {
          if (action === "confirm") {
            ...
            this.copySpellItemEnchantment({ ID: this.currentRow.ID })
              .then(() => {
                Promise.all([
                  this.searchSpellItemEnchantments(this.payload),
                  this.countSpellItemEnchantments(this.payload),
      ...
      this.$confirm(
        ...
        "确认删除",
        {
          ...
          beforeClose: (action, instance, done) => {
            if (action === "confirm") {
              instance.confirmButtonLoading = true;
              this.destroySpellItemEnchantment({ ID: this.currentRow.ID })
                .then(() => {
                  Promise.all([
                    this.searchSpellItemEnchantments(this.payload),
                    this.countSpellItemEnchantments(this.payload),
    ...
    async paginate(page) {
      this.loading = true;
      this.paginateSpellItemEnchantments({ page: page });
      await this.searchSpellItemEnchantments(this.payload);
      this.loading = false;
    },
    show(row) {
      this.$router.push(`/spell-item-enchantment/${row.ID}`);
    },
    async init() {
      this.loading = true;
      await Promise.all([
