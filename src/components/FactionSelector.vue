...
export default {
  ...
  computed: {
    ...
    ...mapState("factionSelector", ["pagination", "factions"]),
    payload() {
      return {
  ...
  methods: {
    ...mapActions("factionSelector", [
      "searchFactionsForSelector",
      "countFactionsForSelector",
      "paginateFactionsForSelector",
    ]),
    input(faction) {
    ...
    async show() {
      this.visible = true;
      await Promise.all([
        this.searchFactionsForSelector(this.payload),
        this.countFactionsForSelector(this.payload),
      ]);
    },
    async search() {
      this.loading = true;
      try {
        this.paginateFactionsForSelector({ page: 1 });
        await Promise.all([
          this.searchFactionsForSelector(this.payload),
          this.countFactionsForSelector(this.payload),
        ]);
    ...
    async paginate(page) {
      this.paginateFactionsForSelector({ page: page });
      await this.searchFactionsForSelector(this.payload);
    },
    select(currentRow) {
