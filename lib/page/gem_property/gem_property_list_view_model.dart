...
class GemPropertyListViewModel with FieldControllerMixin {
  int _refreshToken = 0;
  ...
  Future<void> copyGemProperty(int key) async {
    try {
      final confirmed = await DialogUtil.instance.confirm(
        title: '确认复制',
        description: '是否复制编号为 $key 的宝石属性？',
        confirmText: '复制',
      );
      if (!confirmed) return;
  ...
  Future<void> deleteGemProperty(int key) async {
    try {
      final confirmed = await DialogUtil.instance.confirm(
        title: '确认删除',
        description: '是否删除编号为 $key 的宝石属性？此操作不可撤销。',
        confirmText: '删除',
        destructive: true,
      );
      if (!confirmed) return;
