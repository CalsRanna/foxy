import 'package:foxy/controller/selector_controller.dart';
import 'package:foxy/entity/spell_range_entity.dart';
import 'package:foxy/repository/spell_range_repository.dart';
import 'package:get_it/get_it.dart';

class SpellRangeSelectorController extends SelectorController<SpellRangeEntity> {
  final _repository = GetIt.instance.get<SpellRangeRepository>();

  String idFilter = '';
  String nameFilter = '';

  @override
  String get errorLabel => '搜索射程失败';

  @override
  Future<void> performSearch() async {
    final id = idFilter.isEmpty ? null : idFilter;
    final name = nameFilter.isEmpty ? null : nameFilter;
    final result = await _repository.getSpellRanges(id: id, name: name, page: page);
    final count = await _repository.countSpellRanges(id: id, name: name);
    items = result;
    total = count;
  }

  @override
  void clearFilters() {
    idFilter = '';
    nameFilter = '';
  }
}
