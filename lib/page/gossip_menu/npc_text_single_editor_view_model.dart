...
class NpcTextSingleEditorViewModel with FieldControllerMixin {
  ...
  final entity = signal<NpcTextEntity?>(null);
  ...
  Future<void> _refresh() async {
    ...
    final parentSnapshot = parentKey.value;
    if (parentSnapshot == null || parentSnapshot <= 0) {
      entity.value = null;
      editingKey.value = null;
      localeEditingKey.value = null;
      return;
    ...
    errorMessage.value = null;
    try {
      final main = await _repository.getNpcText(parentSnapshot);
      if (token != _refreshToken) return;
      final candidate = main ?? await _repository.createNpcText(parentSnapshot);
      if (token != _refreshToken) return;
      final localeKey = NpcTextLocaleKey(id: parentSnapshot, locale: 'zhCN');
      final locale = await _localeRepository.getNpcTextLocale(localeKey);
      if (token != _refreshToken) return;
      entity.value = candidate;
      editingKey.value = main == null ? null : parentSnapshot;
