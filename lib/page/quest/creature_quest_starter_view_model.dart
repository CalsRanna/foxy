...
class CreatureQuestStarterViewModel
  ...
  CreatureQuestStarterEntity collectFromForm() => CreatureQuestStarterEntity(
  ...
  Future<bool> create() async {
    try {
      ...
      LoggerUtil.instance.e('创建生物任务开始关系失败: $error');
      DialogUtil.instance.error('创建生物任务开始关系失败: $error');
      return false;
    }
  }
  ...
  Future<void> delete(BuildContext context) async {
    ...
    if (!confirmed) return;
    try {
      await _repository.destroyCreatureQuestStarter(selected.key);
      await _refresh();
  ...
  Future<bool> edit() async {
    ...
    try {
      final existing = await _repository.getCreatureQuestStarter(key);
      if (existing == null) throw StateError('原记录不存在，可能已被修改或删除');
      _initControllers(existing);
      return true;
    } catch (error) {
      _clearEditingState();
      LoggerUtil.instance.e('加载生物任务开始关系失败: $error');
      DialogUtil.instance.error('加载生物任务开始关系失败: $error');
      return false;
    }
  }
  ...
  Future<void> initSignals({required int questId}) async {
    try {
      ...
      LoggerUtil.instance.e('初始化生物任务开始关系失败: $error');
      DialogUtil.instance.error('初始化生物任务开始关系失败: $error');
    }
  }
  ...
  Future<void> persist() async {
    ...
    validateCreatureQuestStarterFields(candidate);
    ...
    editingKey.value = CreatureQuestStarterKey.fromEntity(candidate);
    await _refresh();
  }

  void resetForm() =>
      _initControllers(CreatureQuestStarterEntity(quest: questId.value));
  ...
  void _initControllers(CreatureQuestStarterEntity model) {
  ...
  Future<void> _refresh() async {
    ...
    final count = await _repository.countCreatureQuestStarters(parentQuest);
  ...
  BriefCreatureQuestStarterEntity? get _selectedItem {
