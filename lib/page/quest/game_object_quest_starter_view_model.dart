...
class GameObjectQuestStarterViewModel
  ...
  Future<bool> create() async {
    try {
      ...
      LoggerUtil.instance.e('创建游戏对象任务开始关系失败: $error');
      DialogUtil.instance.error('创建游戏对象任务开始关系失败: $error');
      return false;
    }
  }
  ...
  Future<void> delete(BuildContext context) async {
    ...
    if (!confirmed) return;
    try {
      await _repository.destroyGameObjectQuestStarter(selected.key);
      await _refresh();
  ...
  Future<bool> edit() async {
    ...
    try {
      final existing = await _repository.getGameObjectQuestStarter(key);
      if (existing == null) {
      ...
      _clearEditingState();
      LoggerUtil.instance.e('加载游戏对象任务开始关系失败: $error');
      DialogUtil.instance.error('加载游戏对象任务开始关系失败: $error');
      return false;
    }
  }
  ...
  Future<void> initSignals({required int questId}) async {
    try {
      ...
      LoggerUtil.instance.e('初始化游戏对象任务开始关系失败: $error');
      DialogUtil.instance.error('初始化游戏对象任务开始关系失败: $error');
    }
  }
  ...
  Future<void> persist() async {
    ...
    editingKey.value = GameObjectQuestStarterKey.fromEntity(candidate);
    await _refresh();
  }

  void resetForm() {
    _initControllers(GameObjectQuestStarterEntity(quest: questId.value));
  }
  ...
  void _initControllers(GameObjectQuestStarterEntity model) {
  ...
  Future<void> _refresh() async {
    ...
    final count = await _repository.countGameObjectQuestStarters(parentQuest);
  ...
  BriefGameObjectQuestStarterEntity? get _selectedItem {
