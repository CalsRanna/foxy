...
import 'package:foxy/page/creature_template/gossip_menu_selector_controller.dart';
...
class _GossipMenuSelectorState extends State<GossipMenuSelector> {
  ...
  Future<void> _openDialog() async {
    final controller = GossipMenuSelectorController();
    final currentId = int.tryParse(widget.controller.text) ?? 0;
    if (currentId != 0) {
      controller.menuIdFilter = currentId.toString();
...
class _Dialog extends StatefulWidget {
  final GossipMenuSelectorController controller;
      ...
      child: Row(spacing: 8, children: [
        ...
        Expanded(
          child: Row(spacing: 8, children: [
            ShadButton(
              onPressed: () {
                widget.controller.menuIdFilter = _menuIdController.text;
            ...
            return FoxyShadTable(
              ...
              onRowTap: (row) {
                if (row >= 0 && row < items.length) {
                  widget.controller.select(items[row].menuId);
                }
              },
              onRowDoubleTap: (row) {
                if (row >= 0 && row < items.length) {
