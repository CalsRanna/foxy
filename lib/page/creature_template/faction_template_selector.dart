...
/// 阵营选择器
class FactionTemplateSelector extends StatefulWidget {
  ...
  @override
  State<FactionTemplateSelector> createState() =>
      _FactionTemplateSelectorState();
}

class _FactionTemplateSelectorState extends State<FactionTemplateSelector> {
  @override
  ...
  Future<void> _openDialog() async {
    ...
    final result = await showShadDialog<int>(
      context: context,
      builder: (context) {
        return _FactionTemplateSelectorDialog(initialValue: currentValue);
      },
    );
    if (result != null) {
...
class _FactionTemplateSelectorDialog extends StatefulWidget {
  final int? initialValue;

  const _FactionTemplateSelectorDialog({this.initialValue});

  @override
  State<_FactionTemplateSelectorDialog> createState() =>
      _FactionTemplateSelectorDialogState();
}

class _FactionTemplateSelectorDialogState
    extends State<_FactionTemplateSelectorDialog> {
  final _idController = TextEditingController();

  List<FactionTemplate> _items = [];
  int _total = 0;
  int _page = 1;
  int? _selectedId;
  bool _loading = false;
    ...
    return FoxyShadTable(
      ...
      header: (context, column) {
        return switch (column) {
          ...
          1 => ShadTableCell.header(child: Text('阵营')),
          2 => ShadTableCell.header(child: Text('标识')),
      ...
      builder: (context, vicinity) {
        ...
        return switch (vicinity.column) {
          ...
          1 => ShadTableCell(child: Text(item.faction.toString())),
          2 => ShadTableCell(child: Text(item.flags.toString())),
