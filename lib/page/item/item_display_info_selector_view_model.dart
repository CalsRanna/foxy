...
class ItemDisplayInfoSelectorViewModel {
  final _repository = ItemDisplayInfoRepository();

  final idFilter = signal('');
  final nameFilter = signal('');
  final items = signal<List<ItemDisplayInfoEntity>>([]);
  ...
  Future<void> search() async {
    try {
      ...
      final items = await _repository.getItemDisplayInfos(
        id: id,
        name: name,
        page: page.value,
      );
      final total = await _repository.countItemDisplayInfos(id: id, name: name);
      this.items.value = items;
      this.total.value = total;
    } catch (e) {
      LoggerUtil.instance.e('搜索显示信息失败: $e');
      DialogUtil.instance.error('搜索显示信息失败: $e');
    }
  }
