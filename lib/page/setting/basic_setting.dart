...
class _BasicSettingPageState extends ConsumerState<BasicSettingPage> {
  ...
  Widget _buildData(Setting setting) {
    ...
    final children = [
      ...
      ArcaneInput(controller: hostController),
      const SizedBox(height: 16),
      const Text('端口'),
      const SizedBox(height: 8),
      ArcaneInput(controller: portController),
      const SizedBox(height: 16),
      const Text('用户名'),
      const SizedBox(height: 8),
      ArcaneInput(controller: usernameController),
      const SizedBox(height: 16),
      const Text('密码'),
      const SizedBox(height: 8),
      ArcaneInput(controller: passwordController),
      const SizedBox(height: 16),
      const Text('数据库'),
      const SizedBox(height: 8),
      ArcaneInput(controller: databaseController),
      const SizedBox(height: 16),
      button
    ];
    var column = Column(
