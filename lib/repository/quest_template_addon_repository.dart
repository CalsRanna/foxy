...
class QuestTemplateAddonRepository with RepositoryMixin {
  ...
  /// 根据 ID 查找
  Future<QuestTemplateAddon?> find(int id) async {
    try {
  ...
  Future<QuestTemplateAddon> create(int id) async {
  ...
  /// 存储（insert）
  Future<void> store(QuestTemplateAddon model) async {
    await laconic.table(_table).insert([model.toJson()]);
  }

  /// 更新（根据 ID）
  Future<void> update(int id, QuestTemplateAddon model) async {
