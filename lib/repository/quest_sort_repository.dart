...
class QuestSortRepository with RepositoryMixin {
  ...
  Future<int> count({required QuestSortFilterEntity filter}) async {
    var builder = laconic.table(_table);
    builder = _applyFilter(builder, filter);
    return builder.count();
  }

  Future<QuestSort?> find(int id) async {
    try {
  ...
  Future<void> store(QuestSort data) async {
    await laconic.table(_table).insert([data.toJson()]);
  }

  Future<void> update(QuestSort data) async {
