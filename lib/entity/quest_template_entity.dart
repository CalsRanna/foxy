...
class QuestTemplateEntity {
  ...
  static void _validatePair(String field, int id, int count) {
    if ((id == 0) != (count == 0)) {
      throw ArgumentError('$field 与对应数量必须同时为 0 或同时非 0');
    }
  }

  static void _validateOptionalCount(String field, int id, int count) {
    if (id == 0 && count != 0) {
      throw ArgumentError('$field 为 0 时对应数量也必须为 0');
    }
  }

  static void _validateRewardFaction(
    String field,
    int factionId,
    int value,
    int override,
