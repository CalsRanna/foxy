...
part of 'creature_template_resistance_linked_list_view_model.dart';

mixin _CreatureTemplateResistanceLinkedListViewModelMixin
  ...
  final items = signal(<BriefCreatureTemplateResistanceEntity>[]);

  final editingKey = signal<CreatureTemplateResistanceKey?>(null);

  final selectedKey = signal<CreatureTemplateResistanceKey?>(null);
  ...
  Future<void> persist() async {
    ...
    try {
      ...
      try {
        _logActivity(
          action,
          originalKey ?? CreatureTemplateResistanceKey.fromEntity(candidate),
        );
      } catch (_) {
        // Activity log is best-effort; failure (e.g. not registered in
        // tests) must not affect the main flow.
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(CreatureTemplateResistanceEntity(creatureID: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countCreatureTemplateResistances(link);
