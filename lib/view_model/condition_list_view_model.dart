...
class ConditionListViewModel with FieldControllerMixin, QueryVersionMixin {
  final _repository = GetIt.instance.get<ConditionRepository>();

  final items = signal<List<BriefConditionEntity>>([]);
  ...
  Future<void> _refresh() async {
    ...
    try {
      final (nextItems, nextTotal) = await (
        _repository.getBriefConditions(filter: filter, page: currentPage),
