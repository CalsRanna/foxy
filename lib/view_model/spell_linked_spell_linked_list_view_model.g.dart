...
part of 'spell_linked_spell_linked_list_view_model.dart';

mixin _SpellLinkedSpellLinkedListViewModelMixin on FieldControllerMixin {
  final _repository = GetIt.instance.get<SpellLinkedSpellRepository>();

  final linkKey = signal<int?>(null);

  final items = signal(<BriefSpellLinkedSpellEntity>[]);
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createSpellLinkedSpell(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(SpellLinkedSpellEntity(spellTrigger: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countSpellLinkedSpells(link);
