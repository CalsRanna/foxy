...
part of 'spell_area_linked_list_view_model.dart';

mixin _SpellAreaLinkedListViewModelMixin on FieldControllerMixin {
  ...
  Future<void> create() async {
    ...
    errorMessage.value = null;
    try {
      final candidate = await _repository.createSpellArea(link);
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> persist() async {
    ...
    try {
      ...
      try {
        _logActivity(action, originalKey ?? SpellAreaKey.fromEntity(candidate));
      } catch (_) {
        // 活动日志 best-effort,失败(如测试环境未注册)不影响主流程。
      }
      if (token != _interactionToken || linkKey.value != link) return;
  ...
  Future<void> setLinkKey(int linkKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(SpellAreaEntity(spell: link));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countSpellAreas(link);
      if (token != _refreshToken) return;
      final lastPage = max(1, (count / _repository.kPageSize).ceil());
      final nextPage = min(currentPage, lastPage);
      final data = await _repository.getBriefSpellAreas(link, page: nextPage);
      if (token != _refreshToken) return;
      page.value = nextPage;
      items.value = data;
      total.value = count;
