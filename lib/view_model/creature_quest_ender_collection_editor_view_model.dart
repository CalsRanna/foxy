...
@FoxyDetailViewModel(entity: CreatureQuestEnderEntity)
class CreatureQuestEnderCollectionEditorViewModel
    with
        FieldControllerMixin, _CreatureQuestEnderCollectionEditorViewModelMixin {
  ...
  Future<void> create() async {
    ...
    try {
      final candidate = await _repository.createCreatureQuestEnder(parent);
  ...
  Future<void> setParentKey(int parentKey) async {
    ...
    editingKey.value = null;
    selectedKey.value = null;
    _applyCandidate(CreatureQuestEnderEntity(quest: parent));
    await _refresh();
  }
  ...
  Future<void> _refresh() async {
    ...
    errorMessage.value = null;
    try {
      final count = await _repository.countCreatureQuestEnders(parent);
      if (token != _refreshToken) return;
      final lastPage = max(1, (count / _repository.kPageSize).ceil());
      final nextPage = min(currentPage, lastPage);
      final data = await _repository.getBriefCreatureQuestEnders(parent, page: nextPage);
      if (token != _refreshToken) return;
      page.value = nextPage;
      items.value = data;
      total.value = count;
