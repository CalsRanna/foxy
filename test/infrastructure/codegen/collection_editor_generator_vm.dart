...
void main() {
  test('FoxyCollectionEditorViewModel 生成全套编辑器骨架', () async {
    await testBuilder(
        ...
        'foxy|lib/view_model/child_item_editor_view_model.foxy_view_model.g.part':
            decodedMatches(
              allOf(<Matcher>[
                ...
                contains('final parentKey = signal<int?>(null);'),
                ...
                // 公开方法:copy/create/destroy/edit/persist/setParentKey
                ...
                contains('await _repository.createChildItem(parent);'),
                ...
                contains('Future<void> setParentKey(int parentKey) async {'),
                contains('_applyCandidate(ChildItemEntity(parentId: parent));'),
                // 私有:_refresh 走父键计数 + 列表
                contains('await _repository.countChildItems(parent);'),
  ...
  test('仓库未声明 parentKey 时拒绝生成编辑器骨架', () async {
    ...
    expect(
      logs.any((log) => log.contains('必须声明且只能声明一个 parentKey')),
      isTrue,
    );
  });
}
...
@FoxyRepository(ChildItemEntity, parentKey: ['parentId'])
